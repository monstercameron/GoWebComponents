...
- [x] Add a snapshot-capture short-circuit for unchanged props or declared-source versions before full `ValidateSerializableProps(...)` and source-envelope rebuild (`internal/runtime2/host_region_adapter.go`, `internal/runtime2/spec.go`, `internal/runtime2/snapshot.go`).
  ...
  Notes: `handleHostRegionUpdateSnapshot(...)` now short-circuits props validation only for immutable props (`nil`, bool, numeric scalars, string) through `buildHostRegionSnapshotImmutablePropsToken(...)`; mutable containers (`map[string]any`, `[]any`) intentionally keep per-update `ValidateSerializableProps(...)` to avoid stale token false-hits on in-place mutation. Source snapshot capture now builds the declared-source version tuple through reusable adapter scratch and only materializes a declared-source version map on cache misses, so stable epoch+tuple updates skip source-envelope rebuild and avoid per-update tuple/map allocations on the hit path.
  Validation: `go test ./internal/runtime2 -run "TestHandleHostRegionUpdateSnapshot(CapturesPropsAndSources|ReusesSourceMapForStableVersionTuple|PropsCacheInvalidatesOnInPlaceMutation|InvalidatesSourceMapReuseOnVersionChange|InvalidatesSourceMapReuseOnRemountEpoch)$" -count=1` and `go test ./internal/runtime2 -run ^$ -bench "BenchmarkHandleHostRegionUpdateSnapshotCurrentVsLegacy$" -benchmem -count=5` and `go test ./internal/runtime2 -run ^$ -bench "BenchmarkHandleHostRegion(UpdateSnapshot|ManyHotRegionsBoundedWorkers)$" -benchmem -count=5`
  Validation: `go test -c -o ./bin/runtime2.test.exe ./internal/runtime2` and `./bin/runtime2.test.exe --% -test.run=^$ -test.bench=BenchmarkHandleHostRegionManyHotRegionsBoundedWorkers$ -test.benchtime=10s -test.cpuprofile=./bin/runtime2.hot.cpu.pprof` and `go tool pprof -top ./bin/runtime2.hot.cpu.pprof`
  Result: current-vs-legacy snapshot microbench moved from the pre-change baseline (`current-changed-props` about `1019-1061 ns/op`, `1281-1282 B/op`, `14 allocs/op`; `current-stable-props` about `765-869 ns/op`, `944 B/op`, `12 allocs/op`) to post-change runs (`current-changed-props` about `441-567 ns/op`, `600 B/op`, `4 allocs/op`; `current-stable-props` about `385-463 ns/op`, `592 B/op`, `4 allocs/op`), now ahead of legacy in both modes (`legacy-changed-props` about `556-652 ns/op`, `936 B/op`, `6 allocs/op`; `legacy-stable-props` about `579-686 ns/op`, `928 B/op`, `6 allocs/op`). Pressure benchmark remained in band (`~17.8-22.8 µs/op`, `382-383 B/op`, `47 allocs/op`), and refreshed pprof keeps `handleHostRegionUpdateSnapshot` at roughly `3.79s cum (24.45%)` after this pass.
  Checkpoint: completed todo `Add a snapshot-capture short-circuit for unchanged props or declared-source versions before full ValidateSerializableProps(...) and source-envelope rebuild`; files changed: `internal/runtime2/host_region_adapter.go`, `internal/runtime2/host_region_snapshot_capture_test.go`, `docs/MULTITHREADED_RUNTIME_TODO.md`; validation run: focused snapshot-capture tests, snapshot current-vs-legacy microbench, update-snapshot plus hot-regions pressure bench, and fresh hot-regions pprof capture; result: snapshot capture now short-circuits immutable props safely and trims source-version hit-path overhead with materially lower ns/op and allocs in microbench tracking; residual risk: mutable container props still pay recursive validation cost per update by design, so further gains will likely require a safe prop-shape fingerprinting layer rather than broader tokenization; next suggested todo: `Optimize patch identity hashing in BuildPatchStreamIdentity(...) and JSON streaming hash path`.
...
- [x] Normalize and cache keyed-node presence once during canonical render-node build to remove repeated `strings.TrimSpace(...)` checks in node-ID assignment, string-table extraction, and patch keyed-move detection (`internal/runtime2/render_ir.go`, `internal/runtime2/patch_stream.go`).
  Notes: canonical nodes now cache key-presence (`hasKey`) once during node build and decode (`hasCanonicalNodeKeyText(...)`), then thread that flag through canonical node state and patch insert ops so node-ID path assignment, string-table extraction, keyed-move detection, and insert-node patch encoding no longer re-trim key text in hot loops.
  Validation: `go test ./internal/runtime2 -run "Test(BuildCanonicalRenderIRRootConventions|BuildCanonicalRenderIRStableKeyedNodeIDsAcrossReorder|ParseRenderNodeTable|BuildPatchStreamIdentityMatchesLegacyMarshalEncoding)" -count=1` and `go test ./internal/runtime2 -run ^$ -bench "Benchmark(BuildCanonicalRenderIR|BuildCanonicalPatchStream)$" -benchmem -count=5`
  Checkpoint: completed todo `Normalize and cache keyed-node presence once during canonical render-node build`; files changed: `internal/runtime2/render_ir.go`, `internal/runtime2/patch_stream.go`, `internal/runtime2/perf_parse_canonical_tree_compare_bench_test.go`, `docs/MULTITHREADED_RUNTIME_TODO.md`; result: focused render/table/patch identity tests passed, `BenchmarkBuildCanonicalRenderIR` improved from `~2712-3610 ns/op` to `~2608-2670 ns/op`, and `BenchmarkBuildCanonicalPatchStream` improved from `~7518-9178 ns/op` to `~4940-5031 ns/op`; allocation counts stayed flat (`32` and `45 allocs/op`) while bytes/op increased (`RenderIR 2344 -> 2424`, `PatchStream 7749/7750 -> 8006`) due the additional cached-key presence state carried through hot structs; residual risk: memory footprint per op rose slightly, so follow-up should watch long-list workloads for cache-pressure regressions while validating the throughput win on a pinned runner; next suggested todo: `Optimize diff generation in BuildCanonicalPatchStream(...) and keyed-move ordering helpers`.
...
- [x] Optimize diff generation in `BuildCanonicalPatchStream(...)` and keyed-move ordering helpers (`internal/runtime2/patch_stream.go`).
  Notes: keyed-move planning now short-circuits non-keyed parents with cached canonical `hasKey` and `hasKeyedChild` metadata, so no-keyed sibling spans skip keyed-move index-map construction and move planning loops. Canonical tree decode now computes keyed-child presence once per parent while assembling node state to support that keyed-move fast gate.
  Validation: `go test ./internal/runtime2 -run "Test(BuildCanonicalRenderIRStableKeyedNodeIDsAcrossReorder|BuildCanonicalPatchStreamGeneratesKeyedMove|BuildCanonicalPatchStreamCanonicalOrdering|BuildPatchStreamIdentityMatchesLegacyMarshalEncoding)$" -count=1` and `go test ./internal/runtime2 -run ^$ -bench "BenchmarkBuildCanonicalPatchStream$" -benchmem -count=5`
  Checkpoint: completed todo `Optimize diff generation in BuildCanonicalPatchStream(...) and keyed-move ordering helpers`; files changed: `internal/runtime2/render_ir.go`, `internal/runtime2/patch_stream.go`, `internal/runtime2/perf_parse_canonical_tree_compare_bench_test.go`, `docs/MULTITHREADED_RUNTIME_TODO.md`; result: focused keyed diff and identity tests passed; benchmark on this host remained noisy with same allocations (`45 allocs/op`, `~8005-8006 B/op`) and mixed throughput (`before ~4769-5218 ns/op` vs `after ~5095-7075 ns/op` on the validation run), so net end-to-end speedup is inconclusive here despite the keyed-parent skip path; residual risk: keyed-child metadata computation adds decode work in `ParseCanonicalRenderTree(...)`, so the next pass should either prove net gain with pinned benchstat/pprof or move the guard closer to the diff loop where it costs less when keyed parents dominate; next suggested todo: `Reduce parse-time allocations in ParsePatchStreamTransaction(...), especially mutable lookup-map copy paths`.
...
- [x] Add a FNV-64a pre-filter to `(*HostRegionAdapter).handleHostRegionSnapshotHash(...)` so unchanged snapshot payloads can skip SHA-256 work on the no-change path: compute FNV-64a over the serialized snapshot envelope bytes first; only call the SHA-256 path when the fast hash diverges or when the hex fingerprint string is actually needed (content change) (`internal/runtime2/host_region_adapter.go`, `internal/runtime2/snapshot_dispatch_hash.go`).
  ...
  Checkpoint: completed todo `Add a FNV-64a pre-filter to handleHostRegionSnapshotHash(...)`; files changed: `internal/runtime2/host_region_adapter.go`, `internal/runtime2/perf_host_region_snapshot_hash_prefilter_compare_bench_test.go`, `CHANGELOG.md`, `docs/MULTITHREADED_RUNTIME_TODO.md`; result: new `BenchmarkHandleHostRegionSnapshotHashPrefilterCurrentVsLegacy` confirms no-change path improved from `~1602-1983 ns/op 20 allocs` (legacy SHA always) to `~349-409 ns/op 0 allocs` (FNV prefilter); `BenchmarkHandleHostRegionSnapshotFingerprint/no-change` moved from `~746-1000 ns/op 8 allocs` to `~189-258 ns/op 0 allocs`; residual risk: changed-payload path costs slightly more than legacy SHA due to FNV canonicalization overhead before SHA, but allocations reduced from 31 to 25; next suggested todo: `Split getMutableEntry(...) into a trusted-internal variant`.
...
- [x] Skip `ParseHostRegionDispatchPriority(...)` string-switch validation for internal urgent calls: `HandleHostRegionUpdateDispatch(...)` always passes the `HostRegionDispatchPriorityUrgent` constant to `HandleHostRegionUpdateDispatchWithPriority(...)` which then re-validates via a string switch; add an unexported `handleHostRegionUpdateDispatchWithValidatedPriority(...)` inner form that accepts an already-validated `HostRegionDispatchPriority` value and skips the parse; keep the public method's string validation unchanged for external callers (`internal/runtime2/host_region_adapter.go`).
  ...
  Checkpoint: completed todo `Skip ParseHostRegionDispatchPriority for internal urgent calls`; files changed: `internal/runtime2/host_region_adapter.go`; result: extracted `handleHostRegionUpdateDispatchWithKnownPriority(...)`, `HandleHostRegionUpdateDispatch` and `HandleHostRegionUpdateDispatchWithTransition` now route directly with the known constant, all tests passed, deferred dispatch no-change path improved from `~371-430 ns/op` → `~278-320 ns/op`; residual risk: minimal, the inner function is only reachable from the public wrappers.
