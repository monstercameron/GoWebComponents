...
## Choosing A State Primitive (Decision Guide)

GoWebComponents has one state tool for each distinct job, and they compose rather than
compete. Pick by **where the truth lives and how far it must travel** — the smallest tool
that fits is always the right one. Every primitive below is explicit about its
dependencies; none discovers them through a hidden runtime graph.

| Your situation | Use | Why |
|---|---|---|
| State owned and used by one component | `ui.UseState` | The smallest owner; no sharing machinery. |
...
| Durable local state that survives reloads / syncs across tabs | `kvstate` | Persistence + single-replica conflict resolution. |
| Client-authored data that must converge with a server while offline | `localfirst` | Multi-replica LWW convergence (optimistic + offline + real-time). |
| Structured relational data in the browser | `db/sqlite` | Real SQLite for query-shaped local data. |

Rules of thumb that keep the model coherent:

- **Local before shared.** Reach for an atom only when two *unrelated* branches genuinely
  need the same truth; otherwise `UseState` + context is clearer and cheaper.
- **Server state is not UI state.** Data that comes from a server belongs in `query`
  (cache, dedupe, SWR), not in an atom you hand-refresh — see ch.07. UI state derived from
  it (a selected row, a draft edit) still belongs in `UseState`/atoms.
- **Reach for `Signal` for cost, not style.** Prefer ordinary state until a measured
  re-render is too broad; then bind the hot value with `signal.Text` to update only its
  DOM nodes.
- **`localfirst` is the convergence engine, `kvstate` is the persistence/conflict
