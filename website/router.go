...
// Compiled regex patterns for route matching (compiled once for performance)
var (
	// Hash route pattern: matches /docs, /about, /users/123, etc.
	hashRoutePattern = regexp.MustCompile(`^/([a-zA-Z0-9_-]+)(?:/.*)?$`)

	// Compound hash route pattern: matches /docs#html, /docs#memory, etc.
	compoundHashRoutePattern = regexp.MustCompile(`^/([a-zA-Z0-9_-]+)#([a-zA-Z0-9_-]+)$`)

	// Section link pattern: matches memory, hooks, about, contact, etc.
	sectionLinkPattern = regexp.MustCompile(`^[a-zA-Z0-9_-]+$`)

	// Empty or root pattern: matches empty string or just "/"
...
func Router(props Attrs) *Element {
	...
	GoUseEffect(func() {
		...
		updateRoute := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
			...
			// Route logic using regex pattern matching
			var routeKey string

			// Check for compound hash routes first (e.g., /docs#html, /docs#memory)
			if matches := compoundHashRoutePattern.FindStringSubmatch(hash); matches != nil {
				routeName := matches[1] // Extract the route name from first capture group
				// sectionName := matches[2] // Section name is available but we don't need it for routing
				switch routeName {
				case "docs":
					routeKey = "docs"
				default:
					// Unknown compound route, redirect to app
					js.Global().Get("location").Set("hash", "#")
					return nil
				}
			} else if matches := hashRoutePattern.FindStringSubmatch(hash); matches != nil {
				// Check for simple hash routes (e.g., /docs, /about/section)
				routeName := matches[1] // Extract the route name from capture group
				switch routeName {
				case "docs":
					routeKey = "docs"
				default:
					// Unknown hash route, redirect to app
					js.Global().Get("location").Set("hash", "#")
					return nil
				}
			} else if rootPattern.MatchString(hash) {
				// Empty hash or just "/" goes to app
				routeKey = "app"
			} else if sectionLinkPattern.MatchString(hash) {
				// Section links (memory, hooks, about, etc.) stay on app
				routeKey = "app"
			} else {
				// Invalid pattern, redirect to app
				js.Global().Get("location").Set("hash", "#")
				return nil
			}
...
func TableOfContents(props Attrs) *Element {
	return Div(
		...
		Div(
			...
			TocLink("🎣", "Components & Hooks", "hooks", "GoUseState, GoUseEffect, GoUseMemo, and component lifecycle"),
...
func TocLink(icon, title, sectionId, description string) *Element {
	...
	return Div(
		...
		Div(
			...
			Div(nil,
				H3(Attrs{"class": "font-semibold text-gray-900 group-hover:text-indigo-600"}, title),
				P(Attrs{"class": "text-sm text-gray-600 mt-1"}, description),
			),
		),
	)
}
...
func CoreTypesSection(props Attrs) *Element {
	return Section(
...
func ComponentsHooksSection(props Attrs) *Element {
	return Section(
...
func HTMLElementsSection(props Attrs) *Element {
	return Section(
...
func EventHandlingSection(props Attrs) *Element {
	return Section(
...
func StateManagementSection(props Attrs) *Element {
	return Section(
...
func MemoryManagementSection(props Attrs) *Element {
	return Section(
...
func UtilitiesSection(props Attrs) *Element {
	return Section(
...
func SectionHeader(icon, title, description string) *Element {
	return Div(
		...
		P(Attrs{"class": "text-lg text-gray-600"}, description),
	)
}
	...
	return Div(
		...
		Div(
			...
			Div(
				...
				Div(nil,
					H3(Attrs{"class": "text-xl font-bold text-gray-900"}, name),
					Span(Attrs{"class": "inline-block px-2 py-1 bg-indigo-100 text-indigo-700 text-xs font-medium rounded-full mt-1"}, apiType),
				),
			),
			P(Attrs{"class": "text-gray-600 mt-2"}, description),
		),
		...
		Div(
			...
			P(Attrs{"class": "text-gray-700 leading-relaxed"}, details),
		),
	)
}
...
func HtmlElementGroup(title string, elements []HtmlElementDoc) *Element {
	...
	return Div(
		Attrs{"class": "mb-8"},
		H3(Attrs{"class": "text-xl font-semibold text-gray-900 mb-4"}, title),
		Div(
...
func HtmlElementCard(elem HtmlElementDoc) *Element {
	return Div(
		...
		H4(Attrs{"class": "font-semibold text-gray-900 mb-2"}, elem.Name),
		P(Attrs{"class": "text-sm text-gray-600 mb-3"}, elem.Description),
		Div(
...
// Navigate function for programmatic navigation with pattern-based routing
func Navigate(path string) {
	switch path {
	case "docs":
		// Known hash route - use leading slash
		js.Global().Get("location").Set("hash", "#/docs")
	case "app", "":
		// App route - empty hash
		js.Global().Get("location").Set("hash", "#")
	default:
		// Check if it looks like a hash route pattern (starts with /)
		if strings.HasPrefix(path, "/") {
			// Hash route - use as-is
			js.Global().Get("location").Set("hash", "#"+path)
		} else if sectionLinkPattern.MatchString(path) {
			// Check current location to determine if we need compound hash
			currentHash := js.Global().Get("location").Get("hash").String()
			if strings.HasPrefix(currentHash, "#/docs") {
				// We're on docs page, create compound hash
				js.Global().Get("location").Set("hash", "#/docs#"+path)
			} else {
				// We're on app page, use simple section link
				js.Global().Get("location").Set("hash", "#"+path)
			}
		} else {
			// Invalid pattern, default to app
			js.Global().Get("location").Set("hash", "#")
		}
	}
}
