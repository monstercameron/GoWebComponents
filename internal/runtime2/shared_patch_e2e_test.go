...
// TestSharedPatchPayloadEndToEndCommit verifies shared-buffer patch payload read and host commit end to end.
func TestSharedPatchPayloadEndToEndCommit(parseT *testing.T) {
	...
	parsePatchPayload, parsePatchPayloadErr := BuildStructuredClonePatchPayloadJSON(parseWorkerUpdateResult.PatchIR)
	if parsePatchPayloadErr != nil {
		parseT.Fatalf("BuildStructuredClonePatchPayloadJSON returned error: %v", parsePatchPayloadErr)
	}
	parsePatchEnvelope := StructuredClonePatchEnvelope{
		RegionInstanceID: "region-1",
		Epoch:            1,
		PatchVersion:     parseWorkerUpdateResult.PatchIR.GetHeader.PatchVersion,
	...
	if parseSharedTransportErr != nil {
		parseT.Fatalf("BuildSharedPatchTransportResult returned error: %v", parseSharedTransportErr)
	}
	if parseSharedTransportResult.GetTransportTier != TransportTierSharedBuffer {
		parseT.Fatalf("expected shared-buffer patch transport tier, got %q", parseSharedTransportResult.GetTransportTier)
	}
	...
	if !parseConsumeResult.GetCommitResult.HasCommitted {
		parseT.Fatalf("expected committed shared patch consume result, got %+v", parseConsumeResult)
	}
