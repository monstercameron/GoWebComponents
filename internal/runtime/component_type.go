...
type componentRenderState struct {
	...
	getTypedRender     func(any, any) *Element
	getTypedPropsEqual func(any, any) bool
}
...
// RenderProps invokes the component through its typed-props renderer when the
// element was built by ui.Typed. The ordinary map renderer remains the
// compatibility path for ui.CreateElement and for a mixed construction update.
func (parseComponentType *ComponentType) RenderProps(parseComponentProps map[string]any, parseTypedProps any, hasTypedProps bool) *Element {
	if parseComponentType == nil {
		return nil
	}
	parseStateValue := parseComponentType.getState.Load()
	...
	parseState := parseStateValue.(componentRenderState)
	if parseState.getImplementation == nil {
		return nil
	}
	if hasTypedProps && parseState.getTypedRender != nil {
		return parseState.getTypedRender(parseState.getImplementation, parseTypedProps)
	}
	if parseState.getRender == nil {
		return nil
	}
	return parseState.getRender(parseState.getImplementation, parseComponentProps)
}
...
// SetImplementationTypedRenderer installs the allocation-light renderer used
// by ui.Typed. Typed component props live directly on the element/fiber instead
// of inside a one-entry map allocated for every component instance.
func (parseComponentType *ComponentType) SetImplementationTypedRenderer(parseComponentImplementation any, parseRender func(any, any) *Element) {
	if parseComponentType == nil {
		return
	}
	parseComponentType.setMu.Lock()
	...
	parseCurrentState.getImplementation = parseComponentImplementation
	parseCurrentState.getTypedRender = parseRender
	parseComponentType.getState.Store(parseCurrentState)
}

// SetTypedPropsEqual installs the equality routine generated by ui.Typed for
// its concrete props type. Keeping it beside the renderer in the atomic state
// lets reconciliation avoid reflect.DeepEqual's visited-map machinery for
// structs that contain slices or maps.
func (parseComponentType *ComponentType) SetTypedPropsEqual(parseEqual func(any, any) bool) {
	if parseComponentType == nil {
		return
	}
	parseComponentType.setMu.Lock()
	...
	parseCurrentState.getTypedPropsEqual = parseEqual
	parseComponentType.getState.Store(parseCurrentState)
}

// EqualTypedProps compares two payloads through the concrete comparer
// installed by ui.Typed. handled is false for legacy/mixed component handles.
func (parseComponentType *ComponentType) EqualTypedProps(parseLeft, parseRight any) (isEqual bool, handled bool) {
	if parseComponentType == nil {
		return false, false
	}
	parseStateValue := parseComponentType.getState.Load()
	...
	parseEqual := parseStateValue.(componentRenderState).getTypedPropsEqual
	if parseEqual == nil {
		return false, false
	}
	return parseEqual(parseLeft, parseRight), true
}
