...
func generateRoutesFile(parsePkgName string, parseRoutes []routeDef) (string, error) {
	...
	for _, parseRoute := range parseRoutes {
		...
		fmt.Fprintf(&parseBuilder, "func %s(%s) string {\n", parseFuncName, strings.Join(parseArgList, ", "))
		if len(parseRoute.params) == 0 {
			fmt.Fprintf(&parseBuilder, "\treturn %s.MustHref(nil, nil)\n}\n\n", parseRoute.varName)
			continue
		}
		fmt.Fprintf(&parseBuilder, "\treturn %s.MustHref(map[string]string{\n", parseRoute.varName)
		for parseI, parseParam := range parseRoute.params {
			fmt.Fprintf(&parseBuilder, "\t\t%q: %s,\n", parseParam, parseArgNames[parseI])
		}
