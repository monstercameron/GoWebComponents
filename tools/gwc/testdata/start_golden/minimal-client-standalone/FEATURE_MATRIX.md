...
Preset: `minimal-client` (Minimal Client App)
...
- available `ssr`: Server rendering and hydration expectations are documented in the generated matrix.
...
- available `hydration`: Client boot and hydration ownership are expected from the first app shell.
...
- selected `ui`: Component composition is scaffolded through the public ui package.
- selected `html`: Typed HTML builders are used as the default authoring path.
