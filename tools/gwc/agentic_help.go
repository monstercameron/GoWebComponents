...
func listGwcHelpCommands() []gwcHelpCommand {
	parseCommands := []gwcHelpCommand{
		...
		{Name: "browser", Category: "agent", Summary: "Open a HEADED (visible) Chromium window on a dev URL and keep it open so the engineer can watch the app live while copilot drives it. Exposes a CDP port for `gwc screenshot -cdp` and future click/type tools to attach to this exact window. The browser-proxy backend of gwc mcp for copilot dev. Requires building with -tags playwrightgo.", Usage: "go run -tags playwrightgo ./tools/gwc browser -url http://127.0.0.1:8099 -cdp-port 9222 -json", Examples: []string{"go run -tags playwrightgo ./tools/gwc browser -url http://127.0.0.1:8099 -json", "go run -tags playwrightgo ./tools/gwc browser -url http://127.0.0.1:8099 -cdp-port 9222 -width 1440 -height 900 -json"}, Flags: []gwcHelpFlag{{Name: "url", Type: "string", Description: "URL to open in the visible window (required)"}, {Name: "cdp-port", Type: "int", Default: "9222", Description: "CDP debugging port exposed for copilot attach"}, {Name: "width", Type: "int", Default: "1280", Description: "Window width"}, {Name: "height", Type: "int", Default: "800", Description: "Window height"}, {Name: "wait", Type: "bool", Default: "true", Description: "Keep the window open until interrupted (Ctrl+C)"}, {Name: "json", Type: "bool", Default: "false", Description: "Emit JSON envelope"}}, JSON: true, ReadOnly: true, Experimental: true},
		{Name: "screenshot", Category: "agent", Summary: "Capture a pixel-perfect PNG of a running app via a CDP-driven Chromium (--no-sandbox). Launches its own headless browser, or with -cdp attaches to the engineer's open `gwc browser` window to capture exactly what is on screen. The DevTools/browser-proxy backend of gwc mcp for pixels the in-wasm bridge can't see. Requires building with -tags playwrightgo.", Usage: "go run -tags playwrightgo ./tools/gwc screenshot -url http://127.0.0.1:8099 -out bin/shot.png -json", Examples: []string{"go run -tags playwrightgo ./tools/gwc screenshot -url http://127.0.0.1:8099 -out bin/shot.png -json", "go run -tags playwrightgo ./tools/gwc screenshot -cdp http://127.0.0.1:9222 -out bin/live.png -json", "go run -tags playwrightgo ./tools/gwc screenshot -url http://127.0.0.1:8099 -selector '#app' -out bin/app.png -json"}, Flags: []gwcHelpFlag{{Name: "url", Type: "string", Description: "URL to open and capture (required unless -cdp is set)"}, {Name: "out", Type: "path", Default: "bin/screenshot.png", Description: "Output PNG path"}, {Name: "selector", Type: "string", Description: "Optional CSS selector to capture instead of the full page"}, {Name: "cdp", Type: "string", Description: "Attach to a running browser's CDP endpoint (e.g. http://127.0.0.1:9222) to capture the engineer's open window"}, {Name: "width", Type: "int", Default: "1280", Description: "Viewport width"}, {Name: "height", Type: "int", Default: "800", Description: "Viewport height"}, {Name: "headless", Type: "bool", Default: "true", Description: "Run Chromium headless (ignored when -cdp is set)"}, {Name: "json", Type: "bool", Default: "false", Description: "Emit JSON envelope"}}, JSON: true, ReadOnly: true, Experimental: true},
