...
func listGwcHelpCommands() []gwcHelpCommand {
	parseCommands := []gwcHelpCommand{
		...
		{Name: "test", Category: "verify", Summary: "Run launcher-owned test lanes such as unit, race, wasm, hydration, browser, and release.", Usage: "go run ./tools/gwc test -lane unit -json", Examples: []string{"go run ./tools/gwc test -lane unit -lane race -lane wasm -json", "go run ./tools/gwc test -lane unit -watch"}, Flags: []gwcHelpFlag{{Name: "watch", Type: "bool", Default: "false", Description: "Rerun selected lanes when Go files change"}}, JSON: true, ReadOnly: true},
