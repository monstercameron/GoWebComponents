...
func TestGenerateStartScaffoldWritesStarterFiles(parseT *testing.T) {
	...
	for _, parseExpected6 := range []string{"GoWebComponents Placeholder Brand", "Dark-mode starter scaffold generated by gwc start"} {
...
func TestRunDevExecutesForwardedCommandAndSurfacesFailure(parseT *testing.T) {
	...
	for _, parseExpected := range []string{"run ./tools/livereload/livereload.go", "-app " + parseAppPath, "-root " + parseAppDir, "-html " + parseHtmlPath, "-wasm dist/app.wasm", "-host 0.0.0.0", "-port 9123", "-hot false", "-client-script custom-client.js"} {
