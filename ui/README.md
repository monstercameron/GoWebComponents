...
## SSR Atom Scope (per request, not per process)

Each server render gets its own atom registry. `RenderToString`, `RenderToStringObserved`, `RenderToStream`, and `RenderToStreamObserved` mint one for the duration of that render and discard it afterwards, so:

- `state.UseAtom(id, initial)` seeds from `initial` on **every** request — a previous request can never win.
- A `Set` during a render is visible to the rest of **that** render only.
- Concurrent requests are isolated by construction (no shared registry, nothing to reset).
- Nothing accumulates: a server render never unmounts, so its subscriptions die with its scope instead of piling up in a process-global registry.

Supply request state up front rather than mutating global atoms before rendering:

```go
markup, err := ui.RenderToStringWithAtoms(page(), map[string]any{
    "app.tenant": parseTenant,
    "app.locale": parseLocale,
})
```

Precedence is **seeded value > the component's `UseAtom` initial > nothing**. Streaming takes the same seed through `SSRStreamOptions.InitialAtoms`.

In the browser the opposite semantic holds and is intended: an atom is global to the page and persists across every render.
