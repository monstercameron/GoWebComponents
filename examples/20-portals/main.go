...
func pill(label string) ui.Node {
	return html.Span(html.Props{Class: "rounded-full border border-cyan-500/20 bg-cyan-950/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-100"}, html.Text(label))
}
...
func App() ui.Node {
		...
		overlay = append(overlay, ui.Portal(ui.PortalProps{
			...
			Child: html.Div(html.Props{ID: "modal-surface", Class: "fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-6"},
				html.Div(html.Props{Role: "dialog", Class: "w-full max-w-md rounded-3xl border border-white/10 bg-slate-950/95 p-6 text-slate-100 shadow-2xl"},
					...
					html.P(html.Props{Class: "mt-3 text-sm leading-6 text-slate-300"}, html.Text("This dialog is rendered through ui.Portal into #portal-root rather than inside #app.")),
					html.Div(html.Props{Class: "mt-5 flex gap-3"},
						...
						html.Button(html.Props{ID: "close-modal", Class: "rounded-full border border-white/10 px-4 py-2 text-sm font-semibold text-slate-200", OnClick: ui.UseEvent(func() { showModal.Set(false) })}, html.Text("Close")),
					),
				),
			),
		}))
	...
	if showPopover.Get() {
		overlay = append(overlay, ui.Portal(ui.PortalProps{
			...
			Child: html.Div(html.Props{ID: "popover-surface", Class: "fixed bottom-6 right-6 z-40 w-72 rounded-3xl border border-white/10 bg-slate-950/95 p-5 text-slate-100 shadow-2xl"},
				...
				html.Button(html.Props{ID: "dismiss-popover", Class: "mt-4 rounded-full border border-cyan-500/20 bg-cyan-950/80 px-4 py-2 text-sm font-semibold text-cyan-100", OnClick: ui.UseEvent(func() { showPopover.Set(false) })}, html.Text("Dismiss")),
			),
		}))
	}
	...
	children := []ui.Node{
		html.Div(html.Props{Class: "mx-auto max-w-5xl"},
			...
			html.P(html.Props{Class: "mt-4 max-w-2xl text-base leading-7 text-slate-300"}, html.Text("The controls below live inside #app, while the overlay surfaces render into #portal-root through ui.Portal.")),
			html.Div(html.Props{Class: "mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_320px]"},
				html.Section(html.Props{ID: "app-shell", Class: "rounded-[32px] border border-white/10 bg-slate-950/80 p-6 shadow-[0_20px_80px_rgba(2,6,23,0.45)] backdrop-blur"},
					...
					html.Div(html.Props{Class: "mt-6 flex flex-wrap gap-3"},
						html.Button(html.Props{ID: "open-modal", Class: "rounded-full border border-cyan-500/20 bg-cyan-950/80 px-5 py-3 text-sm font-semibold text-cyan-100", OnClick: ui.UseEvent(func() { showModal.Set(true) })}, html.Text("Open Modal")),
						html.Button(html.Props{ID: "toggle-tooltip", Class: "rounded-full border border-white/10 px-5 py-3 text-sm font-semibold text-slate-200", OnClick: ui.UseEvent(func() { showTooltip.Set(!showTooltip.Get()) })}, html.Text("Toggle Tooltip")),
						html.Button(html.Props{ID: "toggle-popover", Class: "rounded-full border border-white/10 px-5 py-3 text-sm font-semibold text-slate-200", OnClick: ui.UseEvent(func() { showPopover.Set(!showPopover.Get()) })}, html.Text("Toggle Popover")),
					),
					html.P(html.Props{ID: "confirm-count", Class: "mt-6 text-sm font-semibold text-cyan-200"}, html.Text(fmt.Sprintf("Confirmed modal actions: %d", confirmCount.Get()))),
				),
	...
	return html.Div(html.Props{Class: "min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.16),_transparent_26%),linear-gradient(180deg,#08111d_0%,#030712_100%)] px-6 py-10 text-slate-100"}, children...)
}
