...
func productCMSTableShell(items []productAdminCard) ui.Node {
	...
	if len(rows) == 0 {
		rows = append(rows, html.Tag("tr", html.Props{},
			html.Tag("td", html.Props{Class: "px-4 py-6 text-sm text-slate-400", Raw: map[string]interface{}{"colSpan": 8}}, html.Text("No products match the current filter set. Clear filters or create a new Atlas SKU from the route rail.")),
		))
	}
	return html.Div(html.Props{Class: "grid gap-4 " + internalSurfaceCardClass() + " p-5"},
			...
			html.Tag("table", html.Props{Class: "min-w-full border-collapse text-left"},
				html.Tag("thead", html.Props{},
					html.Tag("tr", html.Props{Class: "bg-slate-950/80"},
						...
						html.Tag("th", html.Props{Class: internalTableHeaderCellClass()}, html.Text("Volume")),
						html.Tag("th", html.Props{Class: internalTableHeaderCellClass()}, html.Text("Price")),
...
func productCMSTableRow(item productAdminCard) ui.Node {
	return html.Tag("tr", html.Props{Class: internalTableRowClass() + " align-top"},
		...
		html.Tag("td", html.Props{Class: "px-4 py-4"}, productCMSStatusBadge(item.Status)),
		...
		html.Tag("td", html.Props{Class: "px-4 py-4 text-sm text-slate-200"}, html.Text(fmt.Sprintf("%d", productVolume(item)))),
		html.Tag("td", html.Props{Class: "px-4 py-4 text-sm font-semibold text-white"}, html.Text(formatPrice(item.PriceCents))),
		html.Tag("td", html.Props{Class: "px-4 py-4"},
			html.Div(html.Props{Class: "grid gap-2"},
