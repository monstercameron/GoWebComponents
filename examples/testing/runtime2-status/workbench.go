...
func renderRuntime2StatusWorkbench() ui.Node {
	...
	parseLaneRowNodes := append([]interface{}{
		ClassStr("mt-3 grid grid-cols-4 gap-2 sm:grid-cols-8"),
	}, parseLaneButtons...)
	...
	return Div(
		ClassStr("rounded-[28px] border border-fuchsia-300/20 bg-fuchsia-400/10 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl"),
		P(
			ClassStr("text-xs font-semibold uppercase tracking-[0.24em] text-fuchsia-100"),
		...
		Div(
			ClassStr("mt-5 flex flex-wrap gap-2"),
			Button(
				OnClick(parseSetOverview),
				ClassStr(parsePaneButtonClass(parseActivePaneValue == getRuntime2StatusWorkbenchPaneOverview)),
				Text("Overview"),
			),
			Button(
				OnClick(parseSetSignals),
				ClassStr(parsePaneButtonClass(parseActivePaneValue == getRuntime2StatusWorkbenchPaneSignals)),
				Text("Signals"),
			),
			Button(
				OnClick(parseSetWorkers),
				ClassStr(parsePaneButtonClass(parseActivePaneValue == getRuntime2StatusWorkbenchPaneWorkers)),
				Text("Workers"),
			),
		),
		Div(
			ClassStr("mt-4 rounded-2xl border border-white/10 bg-slate-950/45 p-4"),
			P(
				ClassStr("text-xs font-semibold uppercase tracking-[0.22em] text-slate-400"),
				Text("Step Rail"),
			),
			Div(
				ClassStr("mt-3 flex flex-wrap gap-2"),
				Button(
					OnClick(parseSetStepOne),
					ClassStr(parseStepButtonClass(parseStepValue == 1)),
					Text("x1"),
				),
				Button(
					OnClick(parseSetStepTwo),
					ClassStr(parseStepButtonClass(parseStepValue == 2)),
					Text("x2"),
				),
				Button(
					OnClick(parseSetStepFive),
					ClassStr(parseStepButtonClass(parseStepValue == 5)),
					Text("x5"),
				),
				Button(
					OnClick(parseSetStepEight),
					ClassStr(parseStepButtonClass(parseStepValue == 8)),
					Text("x8"),
				),
				Span(
					ClassStr("ml-1 inline-flex items-center rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-300"),
					Textf("current x%d", parseStepValue),
				),
			),
		),
		Div(
			ClassStr("mt-4 flex flex-wrap gap-2"),
			Button(
				OnClick(parseDecrement),
				ClassStr("rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/[0.08]"),
				Textf("-%d", parseStepValue),
			),
			Button(
				OnClick(parseIncrement),
				ClassStr("rounded-2xl border border-fuchsia-300/30 bg-fuchsia-400/15 px-4 py-3 text-sm font-semibold text-fuchsia-50 transition-colors hover:bg-fuchsia-400/20"),
				Textf("+%d", parseStepValue),
			),
			Button(
				OnClick(parseReset),
			...
			Button(
				...
				ClassStr("rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-semibold text-slate-200 transition-colors hover:bg-white/[0.08]"),
...
func renderRuntime2StatusWorkbenchOverviewSurface(parseCount int, parseTone string, parseStep int) ui.Node {
	return Div(
		ClassStr("rounded-[24px] border border-white/10 bg-white/[0.05] p-4"),
		P(
			ClassStr("text-xs font-semibold uppercase tracking-[0.22em] text-fuchsia-100"),
			Text("Overview Surface"),
		),
		Div(
			ClassStr("mt-4 grid gap-3 sm:grid-cols-3"),
			Div(
				ClassStr("rounded-2xl border border-cyan-300/20 bg-cyan-400/10 p-4"),
				P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-100"), Text("Shared Count")),
				Div(ClassStr("mt-2 text-4xl font-black tracking-tight text-white font-mono"), Textf("%d", parseCount)),
			),
			Div(
				ClassStr("rounded-2xl border border-emerald-300/20 bg-emerald-400/10 p-4"),
				P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-100"), Text("Tone")),
				Div(ClassStr("mt-2 text-2xl font-black tracking-tight text-white"), Text(parseTone)),
			),
			Div(
				ClassStr("rounded-2xl border border-white/10 bg-slate-950/50 p-4"),
				P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400"), Text("Step")),
				Div(ClassStr("mt-2 text-2xl font-black tracking-tight text-white"), Textf("x%d", parseStep)),
			),
		),
		P(
			ClassStr("mt-4 text-sm leading-7 text-slate-300"),
...
func renderRuntime2StatusWorkbenchSignalsSurface(parseCount int, parseTone string, parseStep int, parseSelectedLane int, parseIsRawOpen bool) ui.Node {
	return Div(
		ClassStr("rounded-[24px] border border-white/10 bg-white/[0.05] p-4"),
		P(
			ClassStr("text-xs font-semibold uppercase tracking-[0.22em] text-sky-100"),
			Text("Signals Surface"),
		),
		Div(
			ClassStr("mt-4 grid gap-3 lg:grid-cols-3"),
			Div(
				ClassStr("rounded-2xl border border-white/10 bg-slate-950/55 p-4"),
				P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400"), Text("Pane")),
				Div(ClassStr("mt-2 text-lg font-black tracking-tight text-white"), Text("Local state selector")),
				P(ClassStr("mt-2 text-sm leading-6 text-slate-300"), Text("The active pane is tracked entirely in useState, which makes re-renders easy to force and observe.")),
			),
			Div(
				ClassStr("rounded-2xl border border-white/10 bg-slate-950/55 p-4"),
				P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400"), Text("Payload")),
				Div(ClassStr("mt-2 text-lg font-black tracking-tight text-white"), Textf("count %d", parseCount)),
				P(ClassStr("mt-2 text-sm leading-6 text-slate-300"), Textf("step x%d drives the counter delta and keeps the widget intentionally noisy.", parseStep)),
			),
			Div(
				ClassStr("rounded-2xl border border-white/10 bg-slate-950/55 p-4"),
				P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400"), Text("Lane")),
				Div(ClassStr("mt-2 text-lg font-black tracking-tight text-white"), Textf("focus %d / %d", parseSelectedLane, getRuntime2StatusWorkerCount)),
				P(ClassStr("mt-2 text-sm leading-6 text-slate-300"), Text("The selected lane is a pure UI focus target, not a worker-affinity change.")),
			),
		),
		IfElse(parseIsRawOpen,
			Pre(
				ClassStr("mt-4 overflow-x-auto rounded-2xl border border-white/10 bg-slate-950/60 p-4 text-xs leading-6 text-slate-300"),
				Text(fmt.Sprintf(
					"pane: signals\ncount: %d\ntone: %s\nstep: %d\nfocused lane: %d\nworker pool: %d\nregion: %s\nrenderer: %s",
					parseCount,
					parseTone,
			...
			Div(
				ClassStr("mt-4 rounded-2xl border border-white/10 bg-slate-950/50 p-4"),
				P(
					ClassStr("text-sm leading-7 text-slate-300"),
...
func renderRuntime2StatusWorkbenchWorkersSurface(parseSelectedLane int) ui.Node {
	...
	for parseLane := 1; parseLane <= getRuntime2StatusWorkerCount; parseLane++ {
		...
		parseLaneCards = append(parseLaneCards,
			Div(
				...
				P(
					ClassStr("mt-2 text-xs leading-6 text-slate-300"),
	...
	parseLaneGridNodes := append([]interface{}{
		ClassStr("mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"),
	}, parseLaneCards...)
	...
	return Div(
		...
		Div(
			...
			P(ClassStr("text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400"), Text("Focus Summary")),
			P(
