...
func renderAuthLoadingShell(parseProps authLoadingShellProps) ui.Node {
	...
	if isLoading {
		parseBody = Div(
			ClassStr("space-y-4"),
			Div(
				ClassStr("relative h-[2px] overflow-hidden rounded-full bg-white/[0.08]"),
				Div(
					ClassStr("absolute inset-y-0 left-0 rounded-full bg-[#8e7bff] transition-all duration-300"),
					Style(map[string]string{"width": fmt.Sprintf("%d%%", parsePct)}),
				),
			),
			Div(
				ClassStr("flex items-center justify-between text-sm text-white/35"),
				Span(Text(parseIntl.T(c, "auth.loadingProgress"))),
				Span(ClassStr("text-[11px] uppercase tracking-[0.18em]"), Text(parseIntl.T(c, "auth.loadingPleaseWait"))),
			),
		)
	} else {
		parseBody = Div(
			ClassStr("flex min-h-[84px] items-center justify-between"),
			Div(
				Div(ClassStr("text-sm text-white/[0.38]"), Text(parseIntl.T(c, "auth.loadingFinalizing"))),
				Div(ClassStr("mt-2 text-base font-medium text-white/[0.88]"), Text(parseIntl.T(c, "auth.loadingAlmostReady"))),
			),
			Div(
				ClassStr("relative h-10 w-10"),
				Div(ClassStr("absolute inset-0 rounded-full border border-white/[0.08]"), nil),
				Div(ClassStr("absolute inset-0 animate-spin rounded-full border-2 border-transparent border-t-[#8e7bff]"), nil),
			),
		)
	}

	return Div(
		ClassStr("flex min-h-screen items-center justify-center bg-[#070710] px-6 text-[#f0f0f8]"),
		Div(
			ClassStr("w-full max-w-lg rounded-2xl border border-white/[0.06] bg-[#13131e] p-8 shadow-[0_20px_80px_rgba(0,0,0,0.72)]"),
			Div(
				ClassStr("mb-8 flex items-center justify-between"),
				Div(
					Div(ClassStr("text-[10px] uppercase tracking-[0.35em] text-white/35"), Text(parseIntl.T(c, "auth.loadingBrand"))),
					Div(ClassStr("mt-3 text-xl font-medium tracking-tight text-white/90"), Text(parseIntl.T(c, "auth.loadingLabel"))),
				),
				Div(ClassStr("text-sm tabular-nums text-white/40"), Text(parseDisplayPct)),
			),
			parseBody,
		),
	)
		...
		Div(
			...
			A(
				...
				Div(
					...
					Div(ClassStr("truncate text-[10px] uppercase tracking-[0.16em] text-[#b4b8d0] sm:text-[11px] sm:tracking-[0.18em]"), Text(parseSubtitleText)),
				),
			),
			// actions
			Div(
	...
	return Section(
		...
		Div(
			...
			Div(
				...
				P(ClassStr("mt-5 max-w-[56ch] text-base leading-7 text-[#e6ebf8]/90 sm:mt-6 sm:text-lg sm:leading-8 lg:text-xl"),
					Text(parseBodyText),
				),
				Div(
...
func renderAuthFormCard(parseIntl i18n.Runtime, parseView appViewState, parseAuth authSessionController, isSignup bool) ui.Node {
	...
	return Div(
		...
		Div(
			ClassStr("mb-6"),
			Div(ClassStr("text-sm font-semibold text-[#e0e3f2]"), Text(parseFormSubLabel)),
			H2(ClassStr("mt-2 text-3xl font-semibold tracking-[-0.04em] text-white sm:text-4xl"), Text(parseHeadingText)),
			P(ClassStr("mt-3 text-sm leading-7 text-[#b4b8d0]"), Text(parseSubText)),
		),
		// fields
		Div(
			ClassStr("space-y-5"),
			...
			Div(
				Div(
					ClassStr("mb-2 flex items-center justify-between gap-3"),
					Tag("label",
						ClassStr("block text-sm font-medium text-[#e0e3f2]"),
						For(idAuthPasswordInput),
						Text(parseIntl.T(c, "auth.password")),
					),
					If(!isSignup,
						A(
							ClassStr("text-sm text-[#b4b8d0] transition hover:text-white"),
							Href("#"),
			...
			If(isSignup,
				Tag("label",
					ClassStr("flex items-start gap-3 text-sm text-[#e0e3f2]"),
					Input(Type("checkbox"), ClassStr("mt-1 h-4 w-4 rounded bg-white/10")),
					Span(
						Text(parseIntl.T(c, "auth.termsText")),
						A(ClassStr("text-white transition hover:text-[#f5f7fb]"), Href("#"), Text(parseIntl.T(c, "auth.termsLink"))),
						Text(parseIntl.T(c, "auth.termsAnd")),
						A(ClassStr("text-white transition hover:text-[#f5f7fb]"), Href("#"), Text(parseIntl.T(c, "auth.privacyPolicy"))),
			...
			If(parseView.AuthError != "",
				Div(ID("auth-error-banner"), ClassStr("flex flex-col gap-1 rounded-[18px] border border-red-400/20 bg-red-500/10 px-4 py-3 text-sm leading-6 text-red-100"),
		...
		Div(
			ClassStr("mt-5 flex items-center gap-3"),
			Div(ClassStr("h-px flex-1 bg-white/[0.06]")),
			Span(ClassStr("shrink-0 text-xs text-white/30"), Text(parseIntl.T(c, "auth.orDivider"))),
			Div(ClassStr("h-px flex-1 bg-white/[0.06]")),
		),
		Button(
			ClassStr("mt-3 inline-flex w-full items-center justify-center gap-2.5 rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-medium text-[#e0e3f2] transition hover:bg-white/10"),
			// Google colourised logo mark — inline SVG keeps zero external deps
			Tag("svg",
				Attr("xmlns", "http://www.w3.org/2000/svg"),
				Attr("viewBox", "0 0 24 24"),
				Attr("aria-hidden", "true"),
				ClassStr("h-4 w-4 shrink-0"),
		...
		Div(
			ClassStr("mt-4 rounded-[14px] border border-white/[0.05] bg-white/[0.03] px-4 py-3"),
			Div(ClassStr("mb-1.5 text-[10px] uppercase tracking-[0.16em] text-white/28"), Text("Enterprise")),
			A(
				ClassStr("inline-flex items-center gap-1.5 text-sm text-white/55 transition hover:text-white/85"),
				Href("#"),
				Text(parseIntl.T(c, "auth.ssoEntry")),
				Span(ClassStr("text-white/30"), Text("→")),
			),
		),
		// mode switch footer
		Div(
