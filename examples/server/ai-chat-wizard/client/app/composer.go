...
func parseInputArea(parseProps composerProps) ui.Node {
	...
	return Div(
		ClassStr("shrink-0 px-4 pb-3"),
		Div(ClassStr("max-w-[46rem] mx-auto"),
			If(strings.TrimSpace(parseProps.CueTitle) != "",
				Div(
					ClassStr("mb-2 rounded-2xl border px-4 py-3"),
					ClassStr(parseCueClass),
					Div(ClassStr("text-[0.67rem] font-semibold uppercase tracking-[0.16em]"), Text(parseProps.CueTitle)),
					If(strings.TrimSpace(parseProps.CueBody) != "",
						Div(ClassStr("mt-1 text-sm leading-5"), Text(parseProps.CueBody)),
					),
				),
			),
			Div(
				ID(idChatInputWrap),
				ClassStr("relative flex items-end gap-3 rounded-[1.75rem] border border-white/[0.1] bg-[#17161f] px-5 py-3.5 shadow-[0_16px_48px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,255,255,0.06)] cursor-text transition-[border-color,box-shadow] duration-200 focus-within:border-[#8e7bff]/50 focus-within:shadow-[0_0_0_3px_rgba(142,123,255,0.14),0_16px_48px_rgba(0,0,0,0.55)]"),
				Tag("textarea",
					ID(idChatInput),
					ClassStr("min-h-[1.5rem] flex-1 resize-none bg-transparent text-[1.0625rem] leading-[1.7] text-[#ededf4] placeholder:text-[#7a7d99] focus:outline-none"),
				...
				Div(ClassStr("flex items-center gap-2 shrink-0 self-end"),
					Button(
						ID(idSendBtn),
						ClassStr(ClassNames(
						...
						Span(ClassStr("text-sm leading-none select-none"), Text("\u2191")),
					),
					Tag("span",
						ClassStr("text-white/25 text-base leading-none cursor-default select-none"),
