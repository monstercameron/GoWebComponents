	...
	return Div(
		...
		Div(
			...
			Div(Class("mb-2 flex items-center justify-between px-2"),
				Span(Class("text-[10px] font-semibold uppercase tracking-[0.16em] text-white/30"), Text(parseIntl.T(chatI18nNamespace, "sidebar.threads"))),
				Button(
					Class("rounded-lg border border-white/8 bg-white/[0.04] px-2 py-1 text-[10px] font-medium text-white/45 transition-colors hover:bg-white/[0.08] hover:text-white/70"),
...
func renderSidebarFolderGroup(parseIntl i18n.Runtime, parseGroup sidebarThreadGroup, parseActiveConvID int64, isStreaming bool, parseConversationList conversationListController) ui.Node {
	...
	return Div(
		Class("mt-2 flex flex-col gap-0.5 rounded-xl border border-white/[0.04] bg-white/[0.02] p-1"),
		Attr("draggable", "true"),
		...
		Div(Class("flex items-center gap-1 px-1 py-1"),
			Span(Class("cursor-grab text-white/25"), Text("::")),
			Input(
				Type("text"),
				Value(parseGroup.Folder.Name),
				Data(dataSidebarFolderID, parseFolderID),
				Class("min-w-0 flex-1 rounded-lg border border-transparent bg-transparent px-1.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-white/38 outline-none transition-colors hover:border-white/8 hover:bg-white/[0.03] focus:border-[#8e7bff]/35 focus:bg-white/[0.05] focus:text-white/70"),
			...
			Span(Class("rounded-full bg-white/[0.04] px-1.5 py-0.5 text-[10px] text-white/25"), Textf("%d", len(parseGroup.Threads))),
		),
		If(len(parseGroup.Threads) == 0,
			Div(
				Class("rounded-lg border border-dashed border-white/[0.08] px-2 py-2 text-[11px] text-white/25"),
				Data(dataSidebarFolderID, parseFolderID),
				OnDragOver(parseConversationList.DragOver),
				OnDrop(parseConversationList.DropFolder),
...
func renderSidebarUnfiledGroup(parseIntl i18n.Runtime, parseThreads []convSummary, parseActiveConvID int64, isStreaming bool, parseConversationList conversationListController) ui.Node {
	return Div(
		...
		Div(
			Class("flex items-center justify-between rounded-lg border border-dashed border-white/[0.05] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-white/28"),
			OnDragOver(parseConversationList.DragOver),
			OnDrop(parseConversationList.DropUnfiled),
			Span(Text(parseIntl.T(chatI18nNamespace, "sidebar.unfiled"))),
			Span(Class("rounded-full bg-white/[0.04] px-1.5 py-0.5"), Textf("%d", len(parseThreads))),
		),
...
func renderSidebarThreadRow(parseIntl i18n.Runtime, parseSummary convSummary, parseActiveConvID int64, isStreaming bool, parseFolderID string, parseConversationList conversationListController) ui.Node {
	...
	if parseConversationList.RenameTargetID == parseSummary.ID {
		return Div(
			Class("conv-row flex items-center gap-1 rounded-2xl border border-[#8e7bff]/30 bg-[#8e7bff]/[0.08] px-2 py-1.5 text-sm"),
			Data(dataConvID, parseIdStr),
			Data(dataSidebarFolderID, parseFolderID),
			Input(
				Type("text"),
				Value(parseConversationList.RenameDraft),
				Class("min-w-0 flex-1 rounded-xl border border-white/10 bg-[#0b0a12] px-2 py-1.5 text-xs text-white outline-none focus:border-[#8e7bff]/45"),
			...
			Button(
				Class("rounded-lg px-2 py-1 text-[11px] text-white/45 hover:bg-white/10 hover:text-white/75"),
	...
	return Div(
		...
		Button(
			Class("shrink-0 p-2 rounded-xl opacity-0 group-hover:opacity-100 text-white/35 hover:text-white hover:bg-white/10 transition-all duration-200 ease-out"),
			FromProps(Props{Aria: map[string]string{"label": parseIntl.T(chatI18nNamespace, "sidebar.renameConversation")}}),
			Data(dataConvID, parseIdStr),
		...
		Button(
			Class("shrink-0 p-2 mr-1 rounded-xl opacity-0 group-hover:opacity-100 text-white/35 hover:text-red-400 hover:bg-white/10 transition-all duration-200 ease-out"),
