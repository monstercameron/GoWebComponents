...
func renderLandingShell(parseIntl i18n.Runtime, parseView appViewState, parseAuth authSessionController) ui.Node {
	...
	return Div(
		...
		Main(
			...
			renderSectionDivider("Who this is for"),
			renderLandingPersonaBand(),
			renderSectionDivider("How it works"),
			renderLandingHowItWorksSection(),
			renderSectionDivider(parseIntl.T(n, "divider.capabilities")),
			renderLandingProductSection(parseIntl, parsePage),
			renderSectionDivider("See it in action"),
			renderLandingPreviewCluster(),
			...
			renderLandingFrameworkCalloutStrip(),
		),
