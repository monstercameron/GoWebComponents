...
func canvasWorkspacePane(parseIntl i18n.Runtime, parseSession canvasSessionState, parseController canvasWorkspaceController, isCanvasOnly bool) ui.Node {
	...
	return Div(
		...
		ClassStr("flex h-full min-h-0 min-w-0 flex-col border-l border-white/8 bg-[#101010]"),
		...
		Div(ClassStr("flex items-center gap-2 border-b border-white/8 px-3 py-2"),
			If(isCanvasOnly,
				Button(
					ClassStr(parseMenuButtonClass),
					OnClick(parseController.BackToThread),
					Attr("title", "Back to thread"),
					Text("← Thread"),
				),
			),
			Div(ClassStr("min-w-0 flex-1 flex items-center gap-2 overflow-hidden"),
				Div(ClassStr("truncate text-sm font-semibold text-white/92"), Text(parseSession.CurrentFileID)),
				If(parseSession.Dirty,
					Span(ClassStr("shrink-0 rounded-full border border-[#19c37d]/25 bg-[#19c37d]/12 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[#9af7d0]/72"), Text("edited")),
				),
			),
			// ··· icon — collapses all dev actions so the toolbar feels like a document bar
			Tag("details",
				ClassStr("relative shrink-0"),
				Tag("summary",
					ClassStr("list-none cursor-pointer rounded-md border border-white/10 px-2 py-1 text-[13px] leading-none text-white/50 hover:bg-white/10 hover:text-white/80 transition-colors select-none"),
					Attr("title", "Canvas tools"),
					Text("···"),
				),
				Div(ClassStr("absolute right-0 top-full z-20 mt-1 flex flex-col gap-1 rounded-xl border border-white/10 bg-[#181818] p-2 shadow-xl"),
					Div(ClassStr("mb-1 px-1 text-[10px] uppercase tracking-[0.18em] text-white/30"), Text("Canvas controls")),
					Button(ClassStr(parseMenuButtonClass+" w-full text-left"), OnClick(parseController.Reload), Text("Reload")),
					Button(ClassStr(parseMenuButtonClass+" w-full text-left"), OnClick(parseController.RefreshPreview), Text("Refresh preview")),
					Button(ClassStr(parseMenuButtonClass+" w-full text-left"), OnClick(parseController.ToggleConsole), Text("Toggle console")),
					If(!isCanvasOnly,
						Button(ClassStr(parseMenuButtonClass+" w-full text-left"), OnClick(parseController.ToggleOverlay), Text("Overlay mode")),
					),
					If(!isCanvasOnly,
						Button(ClassStr(parseMenuButtonClass+" w-full text-left"), OnClick(parseController.OpenCanvasOnly), Text("Canvas only")),
					),
				),
			),
			// ✕ icon close — no label, stays visually minimal
			Button(
				ClassStr("shrink-0 rounded-md border border-white/10 px-2 py-1 text-[13px] leading-none text-white/50 hover:bg-white/10 hover:text-white/80 transition-colors"),
		...
		Div(ClassStr("grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[minmax(14rem,0.36fr)_minmax(0,1fr)]"),
			Div(ClassStr("chat-scrollbar flex min-h-0 flex-col overflow-y-auto border-b border-white/8 xl:border-b-0 xl:border-r"),
				Div(ClassStr("border-b border-white/8 px-3 py-2"),
					...
					Div(ClassStr("mt-3 flex flex-wrap items-center gap-2 text-xs text-white/45"),
				...
				Div(ClassStr("grid gap-2 px-3 py-3"),
					Div(ClassStr("rounded-[1.2rem] border border-[#19c37d]/16 bg-[linear-gradient(180deg,rgba(17,34,28,0.72),rgba(10,19,16,0.78))] p-3"),
						Div(ClassStr("mb-2 flex items-center justify-between gap-3"),
							Div(
								ClassStr("text-xs uppercase tracking-[0.18em] text-[#9af7d0]/62"),
								Textf("Focused region: %s", canvasFocusDisplayLabel(parseSession.FocusedRegion)),
							),
							Div(ClassStr("flex items-center gap-2"),
								Button(ClassStr(parseMenuButtonClass), OnClick(parseController.ApplyFocusDraft), Text("Apply patch")),
								Button(ClassStr(parseMenuButtonClass), OnClick(parseController.ResetOriginal), Text("Reset")),
								Button(ClassStr(parseMenuButtonClass), OnClick(parseController.RevertLastPatch), Text("Revert")),
							),
						),
						Tag("textarea",
							ID(idCanvasFocusEditor),
							ClassStr("chat-scrollbar min-h-[10rem] w-full resize-y rounded-2xl border border-white/8 bg-black/20 px-4 py-3 font-mono text-[0.92rem] leading-6 text-white/90 focus:outline-none"),
							Value(parseSession.FocusDraft),
							OnInput(parseController.HandleFocusDraft),
						),
						Div(ClassStr("mt-3 flex flex-wrap items-center gap-2"),
							Button(ClassStr(parseMenuButtonClass), OnClick(parseController.CopyCurrentCode), Text("Copy code")),
							Span(ClassStr("text-xs text-white/40"), Textf("patches: %d", len(parseSession.PatchHistory))),
						),
					),
					Tag("details",
						ClassStr("rounded-[1.25rem] border border-white/8 bg-black/16 p-3"),
						Tag("summary",
							ClassStr("text-xs uppercase tracking-[0.18em] text-white/45"),
							Text("Current file"),
						),
						Div(ClassStr("mt-2"),
					...
					If(len(parseSession.PatchHistory) > 0,
						Tag("details",
							...
							Div(ClassStr("mt-2 flex flex-col gap-2"),
								Map(parseSession.PatchHistory, func(parseRecord canvasPatchRecord) ui.Node {
									return Div(ClassStr("rounded-2xl border border-white/8 bg-white/3 px-3 py-2"),
										Div(ClassStr("text-sm text-white/85"), Text(parseRecord.Summary)),
										Div(ClassStr("mt-1 text-xs text-white/42"), Textf("%s lines %d-%d", parseRecord.Type, parseRecord.StartLine, parseRecord.EndLine)),
									)
								}),
							),
						),
			...
			Div(ClassStr("flex min-h-0 flex-col bg-[#0b0b0b]"),
				Div(ClassStr("flex items-center justify-between gap-3 border-b border-white/8 px-3 py-2"),
					Div(ClassStr("text-xs uppercase tracking-[0.18em] text-white/38"), Text("Preview")),
					Div(ClassStr("text-xs text-white/45"), Textf("render v%d", parseSession.LatestRenderedVersion)),
				),
				Div(ClassStr("min-h-0 flex-1 p-2"),
					Tag("iframe",
						ID(idCanvasFrame),
						ClassStr("h-full min-h-[34rem] w-full rounded-[1rem] border border-black/20 bg-white shadow-[0_24px_70px_rgba(0,0,0,0.28)]"),
		...
		If(parseSession.ConsoleOpen,
			Div(
				ID(idCanvasConsole),
				ClassStr("chat-scrollbar max-h-[10rem] overflow-y-auto border-t border-white/8 bg-black/36 px-3 py-2"),
				Div(ClassStr("mb-3 flex items-center justify-between gap-3"),
					Div(ClassStr("text-xs uppercase tracking-[0.18em] text-white/38"), Text("Console")),
					Button(ClassStr(parseMenuButtonClass), OnClick(parseController.ClearConsole), Text("Clear logs")),
				),
				If(len(parseSession.ConsoleEntries) == 0,
					Div(ClassStr("text-sm text-white/35"), Text("No console output yet.")),
				),
				Div(ClassStr("flex flex-col gap-2"),
					Map(parseSession.ConsoleEntries, func(parseEntry canvasConsoleEntry) ui.Node {
						return Div(ClassStr("rounded-2xl border border-white/8 bg-white/3 px-3 py-2 font-mono text-xs leading-5"),
							Div(ClassStr("flex items-center gap-2"),
								Span(ClassStr("uppercase tracking-[0.18em] text-white/38"), Text(parseEntry.Level)),
								Span(ClassStr("text-white/25"), Textf("render %d", parseEntry.RenderVersion)),
							),
							Div(ClassStr("mt-1 whitespace-pre-wrap text-white/82"), Text(parseEntry.Message)),
							If(strings.TrimSpace(parseEntry.Detail) != "",
								Div(ClassStr("mt-1 whitespace-pre-wrap text-[#ffbdbd]/78"), Text(parseEntry.Detail)),
							),
						)
					}),
				),
...
func canvasSourceView(parseSource string, parseFocus canvasFocusRegion) ui.Node {
	...
	for parseIdx, parseLine := range parseLines {
		...
		parseRows = append(parseRows, Div(
			...
			Span(ClassStr("select-none text-right text-white/26"), Text(strconv.Itoa(parseLineNumber))),
			Span(ClassStr("whitespace-pre-wrap break-words text-white/84"), Text(parseLine)),
		))
	}
	parseRowChildren := make([]interface{}, 0, len(parseRows)+1)
	parseRowChildren = append(parseRowChildren, ClassStr("min-w-full font-mono text-[0.82rem] leading-6"))
	for _, parseRow := range parseRows {
		parseRowChildren = append(parseRowChildren, parseRow)
	}
	return Div(ClassStr("chat-scrollbar max-h-[22rem] overflow-auto rounded-2xl border border-white/8 bg-[#0d0d0d]"),
