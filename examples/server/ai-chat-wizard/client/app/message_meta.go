...
func parseAssistantMessageMetaRow(parseProps assistantMessageMetaProps) ui.Node {
	...
	return Div(ClassStr("mt-1 flex flex-col gap-1"),
		Div(ClassStr("flex items-center gap-2"),
			If(hasExactCost || hasPerformanceStats,
				Div(ClassStr("flex flex-wrap items-center gap-1.5 px-2 py-1 text-xs text-white/45 select-none"),
					If(hasExactCost,
					...
					If(hasExactCost && hasPerformanceStats,
						Span(ClassStr("text-white/15"), Text(".")),
					),
			...
			If(len(parseProps.CanvasArtifacts) > 0,
				Div(ClassStr("flex flex-wrap items-center gap-1"),
					Map(parseProps.CanvasArtifacts, func(parseArtifact canvasArtifact) ui.Node {
						return Button(
							ClassStr("flex items-center gap-1 rounded-lg border border-[#19c37d]/22 px-2 py-1 text-xs text-[#b8ffe0] hover:bg-[#19c37d]/12 transition-colors"),
			...
			Div(ClassStr("ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"),
				Button(
					ClassStr(ClassNames(
				...
				If(!parseProps.TTSStatus.Supported,
					Span(ClassStr("px-2 py-1 text-xs text-white/25 select-none"), Text(parseProps.Intl.T(chatI18nNamespace, "assistant.speechUnavailable"))),
				),
		...
		If(strings.TrimSpace(parseProps.TTSStatus.Error) != "",
			Div(ClassStr("px-2 text-xs text-[#ffb0b0]/80"), Text(parseProps.TTSStatus.Error)),
		),
	)
}
