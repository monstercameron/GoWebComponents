	...
	return Section(
		...
		Div(
			...
			Div(
				...
				P(Class("mt-5 max-w-[52ch] text-base leading-7 text-[#9b9bb1] sm:text-lg sm:leading-8"), Text(parseIntl.T(n, "pricing.hero.body"))),
				Div(
...
func renderPricingStatCard(renderTitle, renderBody string) ui.Node {
	return Div(
		...
		P(Class("mt-2 text-sm leading-6 text-[#9b9bb1]"), Text(renderBody)),
	)
}
		...
		Div(
			...
			Div(
				...
				Map([]pricingPlan{
					...
					if parseBadgeText != "" {
						buildBadge = Div(Class("mb-4 inline-flex rounded-full bg-[#8e7bff]/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8e7bff]"), Text(parseBadgeText))
					}
					parseSuffixText := parseIntl.T(n, parseP.prefix+".suffix")
					var buildSuffix ui.Node
					if parseSuffixText != "" {
						buildSuffix = Span(Class("font-mono-tech text-lg text-[#9b9bb1]"), Text(parseSuffixText))
					}
					...
					return Article(
						...
						buildBadge,
						Div(Class("text-[11px] font-semibold uppercase tracking-[0.14em] text-[#9b9bb1]"), Text(parseIntl.T(n, parseP.prefix+".tone"))),
						...
						P(Class("mt-4 text-sm leading-6 text-[#9b9bb1]"), Text(parseIntl.T(n, parseP.prefix+".description"))),
						Ul(
							Class("mt-6 space-y-2.5 text-sm text-[#9b9bb1]"),
							Map(parseP.featureKeys, func(parseIdx string) ui.Node {
								return Li(
									Class("flex items-start gap-3"),
									Span(Class("mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#8e7bff]"), nil),
...
func renderPricingCompare(parseIntl i18n.Runtime) ui.Node {
	...
	for _, buildColKey := range []string{"compare.col.capability", "compare.col.starter", "compare.col.team", "compare.col.enterprise"} {
		buildGridArgs = append(buildGridArgs, Div(Class("bg-[#13131e] px-4 py-3.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#9b9bb1]"), Text(parseIntl.T(n, buildColKey))))
	}
	for i, buildRowKey := range compareRowKeys {
		...
		buildGridArgs = append(buildGridArgs,
			...
			Div(Class(parseBg+" px-4 py-3.5 text-sm text-[#9b9bb1]"), Text(parseIntl.T(n, buildRowKey+".starter"))),
			Div(Class(parseBg+" px-4 py-3.5 text-sm text-[#9b9bb1]"), Text(parseIntl.T(n, buildRowKey+".team"))),
			Div(Class(parseBg+" px-4 py-3.5 text-sm text-[#9b9bb1]"), Text(parseIntl.T(n, buildRowKey+".enterprise"))),
		)
	}
	...
	return Section(
		...
		Div(
			Class("mx-auto w-[min(1200px,calc(100%-24px))] sm:w-[min(1200px,calc(100%-32px))] lg:w-[min(1200px,calc(100%-40px))]"),
			Div(
				Class("rounded-2xl border border-white/[0.06] bg-[#13131e] px-5 py-7 sm:px-8 sm:py-8 md:px-10 md:py-10 scroll-reveal"),
	...
	return Section(
		...
		Div(
			...
			Div(
				...
				P(Class("mt-4 text-sm leading-7 text-[#9b9bb1]"), Text(parseIntl.T(n, "faq.body"))),
			),
			Div(
				...
				Map(faqKeys, func(parseK string) ui.Node {
					return Div(
						...
						P(Class("mt-3 text-sm leading-6 text-[#9b9bb1]"), Text(parseIntl.T(n, parseK+".a"))),
					)
				}),
			),
		),
	...
	return Section(
		...
		Div(
			...
			Div(
				...
				P(Class("mt-3 text-sm leading-6 text-[#9b9bb1]"), Text("No commitment required. Every path below gives you real answers before rollout.")),
			),
			Div(
				...
				Map(parseGroups, func(parseG parseCTAGroup) ui.Node {
					return Div(
						...
						Div(Class("text-xs font-semibold uppercase tracking-[0.14em] text-[#8e7bff]"), Text(parseG.parseLabel)),
						P(Class("flex-1 text-xs leading-5 text-[#9b9bb1]"), Text(parseG.parseCaption)),
						A(
							Class("inline-flex items-center text-xs font-medium text-[#f0f0f8] hover:text-[#8e7bff] transition-colors"),
...
func renderPricingContact(parseIntl i18n.Runtime) ui.Node {
	...
	return Section(
		...
		Div(
			Class("mx-auto w-[min(1200px,calc(100%-24px))] sm:w-[min(1200px,calc(100%-32px))] lg:w-[min(1200px,calc(100%-40px))]"),
			Div(
				Class("rounded-2xl border border-[#8e7bff]/20 bg-[#13131e] px-6 py-8 sm:px-8 sm:py-10 md:px-10 md:py-12 scroll-reveal"),
				Div(
					Class("max-w-[640px]"),
					...
					P(Class("mt-4 max-w-[52ch] text-base leading-7 text-[#9b9bb1] sm:leading-8"), Text(parseIntl.T(n, "contact.body"))),
					Div(
						Class("mt-8 flex flex-col gap-3 sm:flex-row"),
						A(
							Class("inline-flex items-center justify-center rounded-full bg-[#8e7bff] px-6 py-3.5 text-sm font-semibold text-[#070710] transition hover:-translate-y-[1px]"),
