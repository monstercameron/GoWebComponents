...
- [`sql/store/schema.sql`](C:/Users/Cam/Desktop/GoWebComponents/examples/server/ai-chat-wizard/sql/store/schema.sql) defines `model_catalog` as the source of truth for provider ID, display name, pricing, throughput, onboarding state, and capability flags such as thinking and speech support.
- [`server/app/model_catalog_store.go`](C:/Users/Cam/Desktop/GoWebComponents/examples/server/ai-chat-wizard/server/app/model_catalog_store.go) loads those rows into per-provider catalogs plus the global default, title-generation, and memory-extraction model picks during server startup.
- [`server/app/server.go`](C:/Users/Cam/Desktop/GoWebComponents/examples/server/ai-chat-wizard/server/app/server.go) exposes the runtime query contract through `ListModelOptions`, so the authenticated shell can ask the server which providers and models are currently live.
...
```powershell
cd examples/server/ai-chat-wizard
...
```powershell
go test ./examples/server/ai-chat-wizard/server/app -run '^$' -bench 'Benchmark(StoreCorePaths|SendClientsPerCore)' -benchmem
```
...
go test ./examples/server/ai-chat-wizard/server/app -run TestSendSLASweep -v
```
...
For a fuller explanation of the output fields, interpretation, and latest measured sample data, see [PERFORMANCE.md](C:/Users/Cam/Desktop/GoWebComponents/examples/server/ai-chat-wizard/docs/PERFORMANCE.md).
...
| `CHAT_DB_PATH` | `examples/server/ai-chat-wizard/bin/runtime/chat_history.db` | SQLite database path for auth and conversation persistence |
