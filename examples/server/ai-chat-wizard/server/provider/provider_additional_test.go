...
func TestOpenAIProviderAdditionalMetadataAndUnavailableBranches(parseT *testing.T) {
	...
	if parseInfo := parseProvider.ParseInfo(); parseInfo.ID != "openai" || parseInfo.BaseURL != openAIBaseURL || !parseInfo.Available || !parseInfo.AuthConfigured {
		parseT.Fatalf("Info() = %+v, want available OpenAI metadata", parseInfo)
	}
	if parseHealth := parseProvider.ParseHealth(); parseHealth.ProviderID != "openai" || parseHealth.Status != ProviderHealthUnknown {
	...
	if parseMetadata, parseOk := parseProvider.ParseModelMetadata(" GPT-5.4 "); !parseOk || parseMetadata.ID != "gpt-5.4" {
	...
	if parseFallback := parseProvider.parseMustModelMetadata(" custom-openai "); parseFallback.ID != "custom-openai" || parseFallback.ProviderID != "openai" {
	...
	if _, parseErr4 := parseProvider.ParseSynthesizeSpeech(context.Background(), SpeechRequest{Model: "unknown-model", Text: "hello"}, func(SpeechChunk) error { return nil }); parseErr4 == nil || !strings.Contains(parseErr4.Error(), `does not support speech`) {
...
func TestAnthropicProviderAdditionalMetadataAndHelpers(parseT *testing.T) {
	...
	if parseInfo := parseProvider.ParseInfo(); parseInfo.ID != "anthropic" || parseInfo.BaseURL != anthropicBaseURL || !parseInfo.Available {
		parseT.Fatalf("Info() = %+v, want available anthropic metadata", parseInfo)
	}
	if parseHealth := parseProvider.ParseHealth(); parseHealth.ProviderID != "anthropic" || parseHealth.Status != ProviderHealthUnknown {
		parseT.Fatalf("Health() = %+v, want unknown available health", parseHealth)
	}
	if parseLimits := parseProvider.ParseCurrentRateLimits(); !parseLimits.ParseEmpty() {
		parseT.Fatalf("CurrentRateLimits() = %+v, want empty snapshot", parseLimits)
	}
	if parseMetadata, parseOk := parseProvider.ParseModelMetadata(" Claude-Sonnet-4-5 "); !parseOk || parseMetadata.ID != "claude-sonnet-4-5" {
		parseT.Fatalf("ModelMetadata() = %+v, %t; want claude-sonnet-4-5", parseMetadata, parseOk)
	}
	if parseFallback := parseProvider.parseMustModelMetadata(" custom-claude "); parseFallback.ID != "custom-claude" || parseFallback.ProviderID != "anthropic" {
		parseT.Fatalf("mustModelMetadata() = %+v, want fallback metadata", parseFallback)
	}
	...
	if _, parseErr4 := parseProvider.ParseSynthesizeSpeech(context.Background(), SpeechRequest{Model: "claude-sonnet-4-5", Text: "hello"}, func(SpeechChunk) error { return nil }); parseErr4 == nil || !strings.Contains(parseErr4.Error(), `does not support speech`) {
		parseT.Fatalf("SynthesizeSpeech() error = %v, want unsupported speech capability error", parseErr4)
	}
}
...
func TestStubProviderAdditionalBranches(parseT *testing.T) {
	...
	if parseInfo := parseProvider.ParseInfo(); parseInfo.ID != "openai" || parseInfo.BaseURL != "stub://openai" || parseInfo.AuthConfigured || !parseInfo.Available {
	...
	if parseMetadata, parseOk := parseProvider.ParseModelMetadata("gpt-5.4-mini"); !parseOk || parseMetadata.ID != "gpt-5.4-mini" {
	...
	if parseUnavailable := ParseNewStubProvider("", parseCatalog); parseUnavailable.ParseAvailable() || parseUnavailable.ParseInfo().ID != "" || parseUnavailable.ParseHealth().Status != ProviderHealthUnavailable {
