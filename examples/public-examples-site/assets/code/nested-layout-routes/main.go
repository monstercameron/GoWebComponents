...
func nestedShell(parseTitle string) *router.Element {
	...
	return shared.ExamplePage(
		parseTitle,
		"Nested layout routes with router.UseOutlet()",
		"A layout route stays mounted while more specific child routes render into router.UseOutlet(). That lets shared nav and chrome remain stable across nested navigation.",
		...
		shared.ExamplePanel("Outlet content", html.Div(html.Props{Class: "mt-3"}, router.UseOutlet())),
	)
}
