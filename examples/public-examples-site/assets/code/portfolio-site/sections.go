		...
		Div(
			...
			Div(
				...
				ApiSection(
					...
					[]ApiItem{
						{"Render", "Mount a component tree into the DOM", "selector := exampleboot.GetExampleMountSelector(); ui.Render(ui.CreateElement(App), selector)"},
						{"Hydrate", "Resume a server-rendered tree on the client", "selector := exampleboot.GetExampleMountSelector(); _, _ = ui.Hydrate(ui.CreateElement(App), selector)"},
