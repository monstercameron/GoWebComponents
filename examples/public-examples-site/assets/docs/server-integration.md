...
- CSRF naming helpers and server-shaped form error flows used by `examples/server/server-side-rendering-secure-forms`
- a production-shaped integrated reference server in `examples/server/atlas-commerce-os/server`
...
- `examples/server/server-side-rendering-routing` for route-aware server rendering
- `examples/server/server-side-rendering-secure-forms` for request-time forms, CSRF checks, redirects, and multipart posts
- `examples/server/server-side-rendering-bootstrap` and `examples/public/server-side-rendering-route-data-reuse` for bootstrap transfer and hydration reuse
...
The current first-party reference server is `examples/server/atlas-commerce-os/server`.
...
`examples/server/server-side-rendering-secure-forms` remains the focused form-flow example. Atlas is the production-shaped integrated example.
...
See [DEPLOYMENT_TARGETS.md](deployment-targets-and-adapter-expectations.md) for the short deployment-target summary and adapter expectations.
