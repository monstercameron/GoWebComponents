...
- `examples/public/use-id` shows stable generated IDs for form labeling.
- `examples/public/html-forms` shows typed form controls with explicit label/input wiring.
- `examples/public/form-accessibility` shows validation announcements, `aria-invalid`, `aria-describedby`, pending-state semantics, and shared focus-to-error behavior.
...
- `examples/public/routed-accessibility` shows routed page announcements and heading focus after navigation.
...
- `examples/public/accessible-overlay` shows the modal overlay primitive, focus trap, backdrop dismissal, inert background behavior, and trigger-focus restoration.
- `examples/public/portals` still shows lower-level portal rendering patterns when you do not want the higher-level modal contract.
...
- `examples/public/composite-navigation` shows the same hook applied to both a tablist and a listbox.
