...
This page defines the current browser-support and compatibility policy for GoWebComponents.
...
- evergreen browsers are the current baseline
...
The current minimum support policy is:
...
Older browser versions, long-outdated embedded webviews, and legacy non-evergreen engines are not part of the current support baseline unless a future release explicitly says otherwise.
...
Mobile Safari is part of the current support baseline, not an afterthought.
...
The current progressive-enhancement rules are:
...
This document defines the current baseline and target browser families.
