...
This page records the current boundary for any future server-owned interactive rendering experiment.
...
- request-time SSR through `ui.RenderToString(...)` and hydration through `ui.Hydrate(...)`
- typed bootstrap transfer and SSR observability hooks for the current request-response SSR model
- server-integrated examples that still keep interactivity browser-owned after hydration

Not shipped today:

- server-owned interactive rendering in core
- a patch-stream transport for component updates
- websocket-backed session render loops as part of the supported runtime contract

The current project direction is to keep interactivity browser-owned. If server-interactive rendering is explored at all, it should stay outside the main shipped runtime until it proves a narrower, defensible value.
...
That list is the main reason this topic remains experimental. A server-interactive slice would not be a small transport shim over the shipped browser runtime; it would be a materially different ownership model.
...
If the answer to those questions is no, the right outcome is not a compromised core runtime. The right outcome is to keep the work isolated or stop it.
...
This document intentionally describes an experiment boundary, not a roadmap promise.

It does claim:

- the shipped framework currently treats browser-owned hydration as the interactive model
- a future server-interactive pass would need explicit transport, session, reconnect, and patch ownership
- any first pass should stay small, authenticated, and isolated from the main runtime contract

It does not claim:

- that server-interactive rendering is planned for the default runtime
- that full HTML streaming, tree patches, or DOM-op diffs are approved product direction today
- that the current SSR and hydration APIs are being replaced
