...
- [x] Refresh `examples/public/portfolio-site` to stop teaching stale public APIs.
...
- [x] Add explicit `UseDebounced` versus `UseThrottled` guidance to the text-input example or its README.
	`examples/public/text-input` now explains when to debounce versus throttle so the timing tradeoff is visible in the example itself.
...
- [ ] Make the example inventory and checklist reflect `ui.UseWorkerTask` as a first-class public hook.
	Keep `examples/public/worker-text-index` in the catalog, but also ensure the explicit inventory and backlog coverage treat `ui.UseWorkerTask[...]` as public `ui` surface rather than leaving it discoverable only through the broader `interop` section.
...
- [x] Add a first-class focus-management toolkit.
	`ui.UseFocusManager()` and `ui.UseFocusTrap(...)` now cover focus restoration, focus-to-error helpers, and modal-style focus trapping, with example coverage in `examples/public/accessible-overlay` and `examples/public/form-accessibility`.
- [x] Add keyboard-navigation primitives for composite widgets.
	`ui.UseCompositeNavigation(...)` now provides roving tabindex, arrow-key movement, Home/End handling, typeahead, and active-descendant support, with coverage in `examples/public/composite-navigation` and `ui/ui_wasm_test.go`.
- [x] Add live-region and announcement helpers.
	`ui.UseAnnouncer()` now provides polite and assertive live-region output for validation, async status, and route updates, with coverage in `examples/public/form-accessibility`, `examples/public/routed-accessibility`, and `ui/ui_wasm_test.go`.
...
- [x] Add accessibility-focused examples and tests.
	Added `examples/public/accessible-overlay`, `examples/public/composite-navigation`, `examples/public/form-accessibility`, and `examples/public/routed-accessibility` together with focused Playwright specs and package-level `ui` hook tests.
...
- [x] Add third-party library integration examples.
	`examples/public/web-components` now demonstrates consuming a browser-defined custom element from GoWebComponents, synchronizing reflected attributes and property-only config from Go state, and cleaning up the typed custom-event subscription through effect cleanup.
...
- [x] Explore exporting GoWebComponents components as standards-based custom elements.
	`examples/public/exported-custom-element` now prototypes the current export-side pattern: a browser custom-element wrapper owns the lifecycle and shadow-root target, while Go mounts and unmounts the subtree through `ui.RenderInto(...)`.
...
- [x] Add interoperability examples against third-party web components.
	`examples/public/web-components` now covers consumption of a browser-defined custom-element surface from Go code, and `examples/public/exported-custom-element` covers the separate export-side prototype where plain HTML hosts consume a Go-rendered shadow-root widget.
...
- [x] Add high-level examples that replace raw `syscall/js` usage.
	`examples/public/browser-interop` now demonstrates storage, clipboard access, resize and media observation, grouped DOM lookup, and typed custom-event handling entirely through the public `interop` package.
...
- [x] Add examples for CPU-heavy background work.
	`examples/public/worker-text-index` now demonstrates worker-backed token counting and term indexing with progress updates and cancellation while the main UI remains responsive.
...
- [x] Add protected-route examples and security guidance.
	`examples/public/protected-routes` now demonstrates guarded entry, deferred auth resolution, manual unauthorized fallback UI, and safe post-login redirect preservation, while `docs/ROUTER_AUTH.md` and `router/README.md` now document that client-side gating is a UX tool rather than the real security boundary.
...
- [x] Add route-loader and shared-cache interoperability.
	`fetch.LoadCached[T](...)` now lets route loaders reuse the same shared cache registry as `fetch.UseCachedResource[T](...)`, with the contract documented in `docs/CACHE.md` and exercised in `examples/public/protected-routes`.
...
- [x] Add realistic shared-cache examples for SSR-seeded and route-loader reuse.
	`examples/public/server-side-rendering-cache-bootstrap` now demonstrates SSR-seeded shared cache restore during hydration, and `examples/public/protected-routes` already covers route-loader reuse through `fetch.LoadCached(...)`.
...
- [x] Add browser examples for transition-style UX.
	`examples/public/transition-hooks` now covers typeahead filtering, dashboard tab swaps, and route-style section transitions so `ui.StartTransition(...)` and `ui.UseTransition()` are demonstrated in real app-shaped flows instead of only a toy counter.
...
- [x] Add a server render entrypoint.
	`ui.RenderToString(...)` now provides the public non-browser HTML render entrypoint, and `examples/server/server-side-rendering-routing` exercises request-time SSR over a real Go HTTP server.
	...
	- [x] Add a request-time server-rendered reference example.
		`examples/server/server-side-rendering-routing` now serves HTML per request from a Go HTTP server, emits a route-specific bootstrap sidecar, and hydrates a browser-router client over real URLs.
...
- [x] Add representative multi-target coverage for build experiments.
	`docs/BUILD_EXPERIMENTS.md` now defines the canonical small (`./examples/public/ui-render`), routed mid-sized (`./examples/public/browser-router`), and large showcase (`./examples/server/atlas-commerce-os/client`) wasm targets, and `tools/wasm-build-experiment-targets.json` provides the same list for tooling.
...
- [x] Pilot the shorthand layer on a small set of examples.
	The shorthand surface now has real authored validation beyond package tests: `examples/public/toggle` exercises the reopened second-pass `html/shorthand` path directly, and `examples/public/text-input` covers the form-heavy input path. A dedicated list-heavy shorthand port remains optional follow-up rather than a blocker for the shipped HTML sugar surface.
...
- [x] Add an opinionated starter or reference app once conventions stabilize.
	`examples/server/atlas-commerce-os` is the current production-shaped reference app, and `docs/ONBOARDING.md` now names it as the integrated full-stack reference for larger apps.
...
- [x] Add a first-class lazy asset and module loading pipeline.
	`examples/public/browser-interop` now demonstrates the public module-loading path through `interop.ImportModule(...)`, a browser bridge at `browser-interop.html`, and a module-backed lazy-loading panel that resolves a deferred helper and renders its exports.
...
- [x] Add end-to-end examples for route and component splitting.
	`examples/public/code-splitting` now demonstrates shell-preserving route-family switches with a reloadable lazy panel, and `examples/tests/96-code-splitting.spec.ts` verifies the catalog and operations flows in Playwright.
...
- [x] Add a reference plugin or companion package.
	The repo now includes `head/` as a supported companion package for SSR head composition and `plugin/` plus `examples/public/plugin-host` as the experimental explicit plugin-host reference, both built on documented public APIs rather than privileged runtime internals.
