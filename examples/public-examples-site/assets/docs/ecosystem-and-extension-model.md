...
- current direction: one first-party companion testing module with focused packages such as `test/render`, `test/hooks`, `test/router`, and `test/ssr`; the older `testkit/...` paths remain supported as compatibility aliases; see [TESTING.md](testing-surface.md)
