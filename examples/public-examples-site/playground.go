...
func renderPlaygroundDiagnostic(parseDiagnostic playgroundDiagnostic) ui.Node {
	...
	return Div(
		...
		Class("rounded-[20px] border border-rose-400/25 bg-rose-500/10 p-4 text-sm text-rose-50"),
		Div(Class("flex flex-wrap items-center justify-between gap-2"),
			Span(Class("rounded-full border border-rose-300/30 bg-rose-300/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-rose-100"), Text(parseDiagnostic.Code)),
			Span(Class("text-xs uppercase tracking-[0.16em] text-rose-200/80"), Text(parseLocation)),
		),
		P(Class("mt-3 leading-6"), Text(parseDiagnostic.Message)),
	)
}
...
func renderPlaygroundPreview(parseResult playgroundCompileResult) ui.Node {
	...
	return Iframe(
		...
		Class("h-[340px] w-full rounded-[20px] border border-white/10 bg-white shadow-2xl shadow-black/25"),
	)
}
...
func renderPlaygroundPanel() ui.Node {
	...
	return Section(ID("playground"), Class("mt-2 grid gap-2 rounded-[24px] border border-white/10 bg-slate-950/45 p-3 shadow-2xl shadow-black/20 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.85fr)]"),
		Div(Class("min-w-0"),
			Div(Class("flex flex-wrap items-center justify-between gap-3"),
				Div(
					Div(Class("text-xs uppercase tracking-[0.18em] text-cyan-200"), Text("Snippet playground")),
					H2(Class("mt-1 text-xl font-semibold tracking-tight text-white"), Text("Edit, compile, share")),
				),
				Div(Class("flex flex-wrap gap-2"),
					Button(ID("playground-run"), Type("button"), OnClick(parseRun), Class("rounded-xl border border-emerald-300/25 bg-emerald-400/15 px-3 py-2 text-xs font-medium text-emerald-100 transition hover:bg-emerald-400/20"), Text("Run")),
					Button(Type("button"), OnClick(parseReset), Class("rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-slate-200 transition hover:bg-white/10"), Text("Reset")),
					Button(ID("playground-copy"), Type("button"), OnClick(parseCopy), Class("rounded-xl border border-cyan-300/25 bg-cyan-400/15 px-3 py-2 text-xs font-medium text-cyan-100 transition hover:bg-cyan-400/20"), Text("Copy URL")),
				),
			),
			Textarea(
				...
				Class("mt-3 min-h-[340px] w-full resize-y rounded-[18px] border border-white/10 bg-[#050b14] p-4 font-mono text-sm leading-6 text-cyan-50 outline-none shadow-inner shadow-black/25 placeholder:text-slate-600 focus:border-cyan-300/40"),
			),
			Div(Class("mt-2 flex min-w-0 items-center gap-2 rounded-[16px] border border-white/10 bg-black/20 px-3 py-2 text-xs text-slate-300"),
				Span(Class("shrink-0 text-slate-500"), Text("Share")),
				A(ID("playground-share"), Href(parseShareURL), Class("min-w-0 truncate text-cyan-200 underline decoration-cyan-300/40 underline-offset-4"), Text(parseShareURL)),
				Span(ID("playground-status"), Class("shrink-0 text-slate-500"), Attr("aria-live", "polite"), Text(parseShareStatus.Get())),
			),
		),
		Div(Class("min-w-0"),
			Div(Class("mb-3 flex flex-wrap items-center justify-between gap-2"),
				Div(
					Div(Class("text-xs uppercase tracking-[0.18em] text-slate-500"), Text("Sandbox preview")),
					Div(Class("text-sm text-slate-300"), Text("Isolated iframe output")),
				),
				Span(ID("playground-compile-status"), Attr("data-playground-status", parseCompileLabel), Attr("aria-live", "polite"), Class("rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] "+parseCompileTone), Text(parseCompileLabel)),
			),
