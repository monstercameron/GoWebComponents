...
func portalTargetExample() ui.Node {
	...
	host := js.Global().Get("document").Call("getElementById", explicitPortalRootID)
	...
	page := shared.ExamplePage(
		...
		shared.ExamplePanel("Explicit target",
			...
			html.Div(html.Props{Class: "mt-6 flex flex-wrap gap-3"},
				shared.ExampleButton("Open explicit-node portal", show),
				shared.ExampleButton("Close portal", close),
			),
			html.Div(html.Props{Class: "mt-6 grid gap-4 md:grid-cols-2"},
				shared.ExampleStat("Host resolved", map[bool]string{true: "Yes", false: "No"}[hostReady]),
				shared.ExampleStat("Portal open", map[bool]string{true: "Yes", false: "No"}[open.Get()]),
			),
		),
	)

	if !open.Get() || !hostReady {
		return page
	}
	...
	return ui.Fragment(
		...
		ui.Portal(ui.PortalProps{
			Target: ui.PortalTarget{Node: host},
			Child: html.Div(html.Props{Class: "fixed bottom-8 right-8 z-50 w-96 rounded-[2rem] border border-amber-400/20 bg-slate-950 p-6 text-slate-100 shadow-2xl"},
				...
				html.Div(html.Props{Class: "mt-6 flex gap-3"}, shared.ExampleButton("Dismiss", close)),
			),
		}),
	)
}
