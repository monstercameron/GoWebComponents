...
func TestProviderMetadataHealthAndErrorHelpers(parseT *testing.T) {
	...
	if parseInfo := parseOpenaiConfigured.ParseInfo(); parseInfo.ParseID != "openai" || parseInfo.Label != "OpenAI" || !parseInfo.ParseAvailable || !parseInfo.AuthConfigured || parseInfo.BaseURL != openAIBaseURL {
		parseT.Fatalf("unexpected OpenAI info: %+v", parseInfo)
	}
	if parseHealth := parseOpenaiConfigured.ParseHealth(); parseHealth.ProviderID != "openai" || parseHealth.ParseStatus != ProviderHealthUnknown {
	...
	if parseMetadata, parseOk := parseOpenaiConfigured.ParseModelMetadata(" GPT-5.4-NANO "); !parseOk || parseMetadata.ParseID != "gpt-5.4-nano" {
		parseT.Fatalf("unexpected OpenAI metadata resolution: ok=%v metadata=%+v", parseOk, parseMetadata)
	}
	if parseMetadata2, parseOk2 := parseOpenaiConfigured.ParseModelMetadata("unsupported-model"); parseOk2 || parseMetadata2.ParseID != "" {
		parseT.Fatalf("expected unknown OpenAI model metadata to be unavailable, got ok=%v metadata=%+v", parseOk2, parseMetadata2)
	}
	if parseFallback := parseOpenaiConfigured.parseMustModelMetadata(" custom-model "); parseFallback.ParseID != "custom-model" || parseFallback.DisplayName != "custom-model" || parseFallback.ProviderID != "openai" {
	...
	if parseInfo2 := parseOpenaiUnavailable.ParseInfo(); parseInfo2.ParseAvailable || parseInfo2.AuthConfigured {
		parseT.Fatalf("expected unavailable OpenAI info to reflect missing auth, got %+v", parseInfo2)
	}
	if parseHealth2 := parseOpenaiUnavailable.ParseHealth(); parseHealth2.ParseStatus != ProviderHealthUnavailable {
	...
	if parseInfo3 := parseAnthropicConfigured.ParseInfo(); parseInfo3.ParseID != "anthropic" || parseInfo3.Label != "Anthropic" || !parseInfo3.ParseAvailable || !parseInfo3.AuthConfigured || parseInfo3.BaseURL != anthropicBaseURL {
		parseT.Fatalf("unexpected Anthropic info: %+v", parseInfo3)
	}
	if parseHealth3 := parseAnthropicConfigured.ParseHealth(); parseHealth3.ProviderID != "anthropic" || parseHealth3.ParseStatus != ProviderHealthUnknown {
	...
	if parseMetadata3, parseOk3 := parseAnthropicConfigured.ParseModelMetadata(" CLAUDE-HAIKU-4-5 "); !parseOk3 || parseMetadata3.ParseID != "claude-haiku-4-5" {
		parseT.Fatalf("unexpected Anthropic metadata resolution: ok=%v metadata=%+v", parseOk3, parseMetadata3)
	}
	if parseMetadata4, parseOk4 := parseAnthropicConfigured.ParseModelMetadata("unsupported-model"); parseOk4 || parseMetadata4.ParseID != "" {
		parseT.Fatalf("expected unknown Anthropic model metadata to be unavailable, got ok=%v metadata=%+v", parseOk4, parseMetadata4)
	}
	if parseFallback2 := parseAnthropicConfigured.parseMustModelMetadata(" custom-claude "); parseFallback2.ParseID != "custom-claude" || parseFallback2.DisplayName != "custom-claude" || parseFallback2.ProviderID != "anthropic" {
	...
	if parseInfo4 := parseAnthropicUnavailable.ParseInfo(); parseInfo4.ParseAvailable || parseInfo4.AuthConfigured {
		parseT.Fatalf("expected unavailable Anthropic info to reflect missing auth, got %+v", parseInfo4)
	}
	if parseHealth4 := parseAnthropicUnavailable.ParseHealth(); parseHealth4.ParseStatus != ProviderHealthUnavailable {
		parseT.Fatalf("expected unavailable Anthropic health, got %+v", parseHealth4)
	}

	parseWithModel := (&UnsupportedCapabilityError{Capability: CapabilitySpeech, Model: "gpt-5.4-mini", ProviderID: "openai"}).ParseError()
	...
	parseWithoutModel := (&UnsupportedCapabilityError{Capability: CapabilityThinking, ProviderID: "anthropic"}).ParseError()
	...
	if parseGot := (*UnsupportedCapabilityError)(nil).ParseError(); parseGot != "unsupported capability" {
	...
	if parseGot2 := parseNormalized.ParseError(); !strings.Contains(parseGot2, "openai") || !strings.Contains(parseGot2, "gpt-5.4") || !strings.Contains(parseGot2, "request failed") {
	...
	if parseGot3 := (&NormalizedError{Message: "plain"}).ParseError(); parseGot3 != "plain" {
		parseT.Fatalf("unexpected normalized provider error without provider: %q", parseGot3)
	}
	if parseGot4 := (*NormalizedError)(nil).ParseError(); parseGot4 != "provider error" {
