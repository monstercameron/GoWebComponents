...
WHERE ue.created_at >= ?
  AND LOWER(TRIM(ue.status)) = 'completed'
  AND (ue.prompt_tokens + ue.completion_tokens) >= ?
ORDER BY (ue.prompt_tokens + ue.completion_tokens) DESC, ue.created_at DESC, ue.id DESC
LIMIT ?;
