...
func canvasPreviewPane(parsePreview canvasArtifact) ui.Node {
	...
	return Div(
		...
		Div(Class("flex items-center justify-between gap-3 px-4 py-3 border-b border-white/8"),
			Div(Class("flex flex-col gap-1"),
				Span(Class("text-[0.68rem] uppercase tracking-[0.22em] text-[#9af7d0]/58"), Text(parseIntl.T(chatI18nNamespace, "canvas.badge"))),
				Span(Class("text-sm font-semibold text-white/90"), Text(parseIntl.T(chatI18nNamespace, "canvas.title"))),
			),
			Span(Class("text-[0.75rem] text-white/40"), Text(parseIntl.T(chatI18nNamespace, "canvas.caption"))),
		),
