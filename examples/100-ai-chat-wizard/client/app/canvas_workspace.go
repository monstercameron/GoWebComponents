...
func canvasWorkspacePane(parseIntl i18n.Runtime, parseSession canvasSessionState, parseController canvasWorkspaceController, isCanvasOnly bool) ui.Node {
	...
	parseMetaBadgeClass := "shrink-0 rounded-full border border-white/10 bg-white/4 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-white/45"
	return Div(
		...
		Div(Class("flex items-center gap-2 border-b border-white/8 px-3 py-2"),
			...
			Div(Class("min-w-0 flex-1 flex items-center gap-2 overflow-hidden"),
				...
				Span(Class(parseMetaBadgeClass), Text(parseSession.PreviewStatus)),
				Span(Class(parseMetaBadgeClass), Text(parseSession.RuntimeStatus)),
			...
			Div(Class("chat-scrollbar flex items-center gap-1 overflow-x-auto whitespace-nowrap"),
				Button(Class(parseMenuButtonClass), OnClick(parseController.Reload), Text("Reload")),
				Button(Class(parseMenuButtonClass), OnClick(parseController.RefreshPreview), Text("Refresh")),
				Button(Class(parseMenuButtonClass), OnClick(parseController.ToggleConsole), Text("Console")),
				If(!isCanvasOnly,
					Button(Class(parseMenuButtonClass), OnClick(parseController.ToggleOverlay), Text("Overlay")),
				),
				If(!isCanvasOnly,
					Button(Class(parseMenuButtonClass), OnClick(parseController.OpenCanvasOnly), Text("Canvas only")),
				),
				Button(Class(parseMenuButtonClass), OnClick(parseController.Close), Text("Close")),
			),
		),
