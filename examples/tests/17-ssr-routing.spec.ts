...
test.describe('17-SSR Routing', () => {
  ...
  test('hydrates and drives advanced routing flows', async ({ page }) => {
    ...
    await expect(page.getByText('Nested async panel ready', { exact: true })).toBeVisible({ timeout: 5000 });
    await expect(page.getByText('Loader key: docs:routing:loader', { exact: false })).toBeVisible();
    ...
    await expect(page.getByText('Loader key: search:all', { exact: false })).toBeVisible({ timeout: 10000 });
    await page.getByRole('button', { name: 'Filter routing' }).click();
    await expect(page).toHaveURL(/ssr-routing\.html#\/search\?q=routing$/);
    await expect(page.getByText('Current query: routing', { exact: true })).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('Loader key: search:routing', { exact: false })).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByText('Loader key: secure:Morgan Reconciler:maintainer', { exact: false })).toBeVisible({ timeout: 10000 });
  });
});