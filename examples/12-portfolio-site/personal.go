		...
		dom.Div(
			...
			dom.Div(
				...
				dom.H1(
					...
					dom.Span(Attrs{"class": "bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent"}, "Earl Cameron"),
				),
				dom.H2(
...
func PersonalStatCard(number, label string) *Element {
	return dom.Div(
		...
		dom.P(Attrs{"class": "text-3xl font-bold text-blue-400 mb-1"}, number),
		dom.P(Attrs{"class": "text-sm text-gray-400"}, label),
	)
}
				...
				dom.Div(
					...
					dom.Div(
						...
						dom.Div(
							...
							dom.Div(Attrs{"class": "absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent rounded-2xl"}),
							...
							dom.Div(
								...
								dom.P(Attrs{"class": "text-sm opacity-90 drop-shadow-md text-blue-300"}, "Full-Stack Engineer"),
								dom.P(Attrs{"class": "text-xs opacity-75 drop-shadow-md"}, "AI/ML Enthusiast & Framework Creator"),
							),
						),

						// Floating accent elements
						dom.Div(Attrs{"class": "absolute -top-4 -right-4 w-24 h-24 bg-blue-500/20 rounded-full blur-xl animate-pulse"}),
						dom.Div(Attrs{"class": "absolute -bottom-4 -left-4 w-24 h-24 bg-purple-500/20 rounded-full blur-xl animate-pulse delay-1000"}),
					),
				),
			),
		),
...
func PersonalHighlightItem(icon, title, description string) *Element {
	return dom.Div(
		...
		dom.Div(
			...
			dom.P(Attrs{"class": "text-sm text-gray-400"}, description),
		),
	)
}
